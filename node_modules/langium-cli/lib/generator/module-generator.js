...
export function generateModule(grammars, config, grammarConfigMap) {
    const grammarsWithName = grammars.filter(grammar => !!grammar.name);
    const parserConfig = config.chevrotainParserConfig;
    const mode = config.mode;
    const hasIParserConfigImport = Boolean(parserConfig) || grammars.some(grammar => { var _a; return ((_a = grammarConfigMap.get(grammar)) === null || _a === void 0 ? void 0 : _a.chevrotainParserConfig) !== undefined; });
    let needsGeneralParserConfig = undefined;
    /* eslint-disable @typescript-eslint/indent */
    const node = expandToNode `
        ${generatedHeader}
    `.appendNewLine().appendIf(!!config.langiumInternal, expandToNode `
...
function generateParserConfig(config) {
    return joinToNode(Object.entries(config), ([key, value]) => `${key}: ${typeof value === 'string' ? `'${value}'` : value},`, { appendNewLineIfNotEmpty: true });
}
