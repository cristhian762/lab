import { D as LA, E as IB, S as DB, a as fA, T as wB, k as TA, n as oB, c as sB, b as iB, d as WA, g as XA, e as eB, f as cB, C as OA, L as tB, h as MB, R as hB, o as vA, i as rA, j as ZA, F as aA, l as aB, t as FB, U as nB, I as zA, m as GB, p as NB, q as VA, s as jA, u as LB, v as _A, w as $A, x as JA, y as xA, z as HB, A as kB, B as z, G as rB, H as UA, J as AB, K as RB, M as KB, N as YB, O as UB, P as PB, Q as qA, V as JB, W as yB, X as SB, Y as uB, Z as pB, _ as dB, $ as BB, a0 as lB, a1 as fB, a2 as OB, a3 as QB, a4 as xB, a5 as qB, a6 as mB, a7 as bB, a8 as TB } from "./index-8r3crY85.js";
function gB(p, c) {
  for (var L = 0; L < c.length; L++) {
    const r = c[L];
    if (typeof r != "string" && !Array.isArray(r)) {
      for (const K in r)
        if (K !== "default" && !(K in p)) {
          const Q = Object.getOwnPropertyDescriptor(r, K);
          Q && Object.defineProperty(p, K, Q.get ? Q : {
            enumerable: !0,
            get: () => r[K]
          });
        }
    }
  }
  return Object.freeze(Object.defineProperty(p, Symbol.toStringTag, { value: "Module" }));
}
function pA(p, c, L, r) {
  var K = arguments.length, Q = K < 3 ? c : r === null ? r = Object.getOwnPropertyDescriptor(c, L) : r, N;
  if (typeof Reflect == "object" && typeof Reflect.decorate == "function")
    Q = Reflect.decorate(p, c, L, r);
  else
    for (var D = p.length - 1; D >= 0; D--)
      (N = p[D]) && (Q = (K < 3 ? N(Q) : K > 3 ? N(c, L, Q) : N(c, L)) || Q);
  return K > 3 && Q && Object.defineProperty(c, L, Q), Q;
}
function V(p, c) {
  return function(L, r) {
    c(L, r, p);
  };
}
class WB extends LA {
  constructor(c, L, r, K, Q, N, D) {
    super(), this._grammar = c, this._initialState = L, this._containsEmbeddedLanguages = r, this._createBackgroundTokenizer = K, this._backgroundTokenizerShouldOnlyVerifyTokens = Q, this._reportTokenizationTime = N, this._reportSlowTokenization = D, this._seenLanguages = [], this._onDidEncounterLanguage = this._register(new IB()), this.onDidEncounterLanguage = this._onDidEncounterLanguage.event;
  }
  get backgroundTokenizerShouldOnlyVerifyTokens() {
    return this._backgroundTokenizerShouldOnlyVerifyTokens();
  }
  getInitialState() {
    return this._initialState;
  }
  tokenize(c, L, r) {
    throw new Error("Not supported!");
  }
  createBackgroundTokenizer(c, L) {
    if (this._createBackgroundTokenizer)
      return this._createBackgroundTokenizer(c, L);
  }
  tokenizeEncoded(c, L, r) {
    const K = Math.random() * 1e4 < 1, Q = this._reportSlowTokenization || K, N = Q ? new DB(!0) : void 0, D = this._grammar.tokenizeLine2(c, r, 500);
    if (Q) {
      const g = N.elapsed();
      (K || g > 32) && this._reportTokenizationTime(g, c.length, K);
    }
    if (D.stoppedEarly)
      return console.warn(`Time limit reached when tokenizing line: ${c.substring(0, 100)}`), new fA(D.tokens, r);
    if (this._containsEmbeddedLanguages) {
      const g = this._seenLanguages, G = D.tokens;
      for (let i = 0, t = G.length >>> 1; i < t; i++) {
        const Y = G[(i << 1) + 1], a = wB.getLanguageId(Y);
        g[a] || (g[a] = !0, this._onDidEncounterLanguage.fire(a));
      }
    }
    let F;
    return r.equals(D.ruleStack) ? F = r : F = D.ruleStack, new fA(D.tokens, F);
  }
}
class XB extends LA {
  get backgroundTokenizerShouldOnlyVerifyTokens() {
    return this._actual.backgroundTokenizerShouldOnlyVerifyTokens;
  }
  constructor(c, L, r) {
    super(), this._encodedLanguageId = c, this._actual = L, this._maxTokenizationLineLength = r, this._register(TA(this._maxTokenizationLineLength));
  }
  getInitialState() {
    return this._actual.getInitialState();
  }
  tokenize(c, L, r) {
    throw new Error("Not supported!");
  }
  tokenizeEncoded(c, L, r) {
    return c.length >= this._maxTokenizationLineLength.get() ? oB(this._encodedLanguageId, r) : this._actual.tokenizeEncoded(c, L, r);
  }
  createBackgroundTokenizer(c, L) {
    if (this._actual.createBackgroundTokenizer)
      return this._actual.createBackgroundTokenizer(c, L);
  }
}
class vB {
  constructor(c) {
    this.edits = c.slice().sort(sB((L) => L.offset, iB));
  }
  applyToArray(c) {
    for (let L = this.edits.length - 1; L >= 0; L--) {
      const r = this.edits[L];
      c.splice(r.offset, r.length, ...new Array(r.newLength));
    }
  }
}
class ZB {
  constructor(c, L, r) {
    this.offset = c, this.length = L, this.newLength = r;
  }
  toString() {
    return `[${this.offset}, +${this.length}) -> +${this.newLength}}`;
  }
}
class RA {
  static fromMany(c) {
    const L = c.map((r) => new RA(r));
    return new zB(L);
  }
  constructor(c) {
    this.transformation = c, this.idx = 0, this.offset = 0;
  }
  transform(c) {
    let L = this.transformation.edits[this.idx];
    for (; L && L.offset + L.length <= c; )
      this.offset += L.newLength - L.length, this.idx++, L = this.transformation.edits[this.idx];
    if (!(L && L.offset <= c))
      return c + this.offset;
  }
}
class zB {
  constructor(c) {
    this.transformers = c;
  }
  transform(c) {
    for (const L of this.transformers) {
      const r = L.transform(c);
      if (r === void 0)
        return;
      c = r;
    }
    return c;
  }
}
var EB = { exports: {} };
(function(p, c) {
  (function(L, r) {
    p.exports = r();
  })(WA, function() {
    return (() => {
      var L = { 350: (K, Q) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.UseOnigurumaFindOptions = Q.DebugFlags = void 0, Q.DebugFlags = { InDebugMode: typeof process < "u" && !!process.env.VSCODE_TEXTMATE_DEBUG }, Q.UseOnigurumaFindOptions = !1;
      }, 442: (K, Q, N) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.applyStateStackDiff = Q.diffStateStacksRefEq = void 0;
        const D = N(391);
        Q.diffStateStacksRefEq = function(F, g) {
          let G = 0;
          const i = [];
          let t = F, Y = g;
          for (; t !== Y; )
            t && (!Y || t.depth >= Y.depth) ? (G++, t = t.parent) : (i.push(Y.toStateStackFrame()), Y = Y.parent);
          return { pops: G, newFrames: i.reverse() };
        }, Q.applyStateStackDiff = function(F, g) {
          let G = F;
          for (let i = 0; i < g.pops; i++)
            G = G.parent;
          for (const i of g.newFrames)
            G = D.StateStackImpl.pushFrame(G, i);
          return G;
        };
      }, 36: (K, Q) => {
        var N;
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.toOptionalTokenType = Q.EncodedTokenAttributes = void 0, (N = Q.EncodedTokenAttributes || (Q.EncodedTokenAttributes = {})).toBinaryStr = function(D) {
          let F = D.toString(2);
          for (; F.length < 32; )
            F = "0" + F;
          return F;
        }, N.print = function(D) {
          const F = N.getLanguageId(D), g = N.getTokenType(D), G = N.getFontStyle(D), i = N.getForeground(D), t = N.getBackground(D);
          console.log({ languageId: F, tokenType: g, fontStyle: G, foreground: i, background: t });
        }, N.getLanguageId = function(D) {
          return (255 & D) >>> 0;
        }, N.getTokenType = function(D) {
          return (768 & D) >>> 8;
        }, N.containsBalancedBrackets = function(D) {
          return (1024 & D) != 0;
        }, N.getFontStyle = function(D) {
          return (30720 & D) >>> 11;
        }, N.getForeground = function(D) {
          return (16744448 & D) >>> 15;
        }, N.getBackground = function(D) {
          return (4278190080 & D) >>> 24;
        }, N.set = function(D, F, g, G, i, t, Y) {
          let a = N.getLanguageId(D), k = N.getTokenType(D), H = N.containsBalancedBrackets(D) ? 1 : 0, e = N.getFontStyle(D), I = N.getForeground(D), M = N.getBackground(D);
          return F !== 0 && (a = F), g !== 8 && (k = g), G !== null && (H = G ? 1 : 0), i !== -1 && (e = i), t !== 0 && (I = t), Y !== 0 && (M = Y), (a << 0 | k << 8 | H << 10 | e << 11 | I << 15 | M << 24) >>> 0;
        }, Q.toOptionalTokenType = function(D) {
          return D;
        };
      }, 996: (K, Q, N) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.BasicScopeAttributesProvider = Q.BasicScopeAttributes = void 0;
        const D = N(878);
        class F {
          constructor(t, Y) {
            this.languageId = t, this.tokenType = Y;
          }
        }
        Q.BasicScopeAttributes = F;
        class g {
          constructor(t, Y) {
            this._getBasicScopeAttributes = new D.CachedFn((a) => {
              const k = this._scopeToLanguage(a), H = this._toStandardTokenType(a);
              return new F(k, H);
            }), this._defaultAttributes = new F(t, 8), this._embeddedLanguagesMatcher = new G(Object.entries(Y || {}));
          }
          getDefaultAttributes() {
            return this._defaultAttributes;
          }
          getBasicScopeAttributes(t) {
            return t === null ? g._NULL_SCOPE_METADATA : this._getBasicScopeAttributes.get(t);
          }
          _scopeToLanguage(t) {
            return this._embeddedLanguagesMatcher.match(t) || 0;
          }
          _toStandardTokenType(t) {
            const Y = t.match(g.STANDARD_TOKEN_TYPE_REGEXP);
            if (!Y)
              return 8;
            switch (Y[1]) {
              case "comment":
                return 1;
              case "string":
                return 2;
              case "regex":
                return 3;
              case "meta.embedded":
                return 0;
            }
            throw new Error("Unexpected match for standard token type!");
          }
        }
        Q.BasicScopeAttributesProvider = g, g._NULL_SCOPE_METADATA = new F(0, 0), g.STANDARD_TOKEN_TYPE_REGEXP = /\b(comment|string|regex|meta\.embedded)\b/;
        class G {
          constructor(t) {
            if (t.length === 0)
              this.values = null, this.scopesRegExp = null;
            else {
              this.values = new Map(t);
              const Y = t.map(([a, k]) => D.escapeRegExpCharacters(a));
              Y.sort(), Y.reverse(), this.scopesRegExp = new RegExp(`^((${Y.join(")|(")}))($|\\.)`, "");
            }
          }
          match(t) {
            if (!this.scopesRegExp)
              return;
            const Y = t.match(this.scopesRegExp);
            return Y ? this.values.get(Y[1]) : void 0;
          }
        }
      }, 947: (K, Q, N) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.LineTokens = Q.BalancedBracketSelectors = Q.StateStackImpl = Q.AttributedScopeStack = Q.Grammar = Q.createGrammar = void 0;
        const D = N(350), F = N(36), g = N(736), G = N(44), i = N(792), t = N(583), Y = N(878), a = N(996), k = N(47);
        function H(C, B, s, o, n) {
          const R = g.createMatchers(B, e), P = i.RuleFactory.getCompiledRuleId(s, o, n.repository);
          for (const y of R)
            C.push({ debugSelector: B, matcher: y.matcher, ruleId: P, grammar: n, priority: y.priority });
        }
        function e(C, B) {
          if (B.length < C.length)
            return !1;
          let s = 0;
          return C.every((o) => {
            for (let n = s; n < B.length; n++)
              if (I(B[n], o))
                return s = n + 1, !0;
            return !1;
          });
        }
        function I(C, B) {
          if (!C)
            return !1;
          if (C === B)
            return !0;
          const s = B.length;
          return C.length > s && C.substr(0, s) === B && C[s] === ".";
        }
        Q.createGrammar = function(C, B, s, o, n, R, P, y) {
          return new M(C, B, s, o, n, R, P, y);
        };
        class M {
          constructor(B, s, o, n, R, P, y, S) {
            if (this._rootScopeName = B, this.balancedBracketSelectors = P, this._onigLib = S, this._basicScopeAttributesProvider = new a.BasicScopeAttributesProvider(o, n), this._rootId = -1, this._lastRuleId = 0, this._ruleId2desc = [null], this._includedGrammars = {}, this._grammarRepository = y, this._grammar = h(s, null), this._injections = null, this._tokenTypeMatchers = [], R)
              for (const W of Object.keys(R)) {
                const q = g.createMatchers(W, e);
                for (const d of q)
                  this._tokenTypeMatchers.push({ matcher: d.matcher, type: R[W] });
              }
          }
          get themeProvider() {
            return this._grammarRepository;
          }
          dispose() {
            for (const B of this._ruleId2desc)
              B && B.dispose();
          }
          createOnigScanner(B) {
            return this._onigLib.createOnigScanner(B);
          }
          createOnigString(B) {
            return this._onigLib.createOnigString(B);
          }
          getMetadataForScope(B) {
            return this._basicScopeAttributesProvider.getBasicScopeAttributes(B);
          }
          _collectInjections() {
            const B = [], s = this._rootScopeName, o = ((n) => n === this._rootScopeName ? this._grammar : this.getExternalGrammar(n))(s);
            if (o) {
              const n = o.injections;
              if (n)
                for (let P in n)
                  H(B, P, n[P], this, o);
              const R = this._grammarRepository.injections(s);
              R && R.forEach((P) => {
                const y = this.getExternalGrammar(P);
                if (y) {
                  const S = y.injectionSelector;
                  S && H(B, S, y, this, y);
                }
              });
            }
            return B.sort((n, R) => n.priority - R.priority), B;
          }
          getInjections() {
            if (this._injections === null && (this._injections = this._collectInjections(), D.DebugFlags.InDebugMode && this._injections.length > 0)) {
              console.log(`Grammar ${this._rootScopeName} contains the following injections:`);
              for (const B of this._injections)
                console.log(`  - ${B.debugSelector}`);
            }
            return this._injections;
          }
          registerRule(B) {
            const s = ++this._lastRuleId, o = B(i.ruleIdFromNumber(s));
            return this._ruleId2desc[s] = o, o;
          }
          getRule(B) {
            return this._ruleId2desc[i.ruleIdToNumber(B)];
          }
          getExternalGrammar(B, s) {
            if (this._includedGrammars[B])
              return this._includedGrammars[B];
            if (this._grammarRepository) {
              const o = this._grammarRepository.lookup(B);
              if (o)
                return this._includedGrammars[B] = h(o, s && s.$base), this._includedGrammars[B];
            }
          }
          tokenizeLine(B, s, o = 0) {
            const n = this._tokenize(B, s, !1, o);
            return { tokens: n.lineTokens.getResult(n.ruleStack, n.lineLength), ruleStack: n.ruleStack, stoppedEarly: n.stoppedEarly };
          }
          tokenizeLine2(B, s, o = 0) {
            const n = this._tokenize(B, s, !0, o);
            return { tokens: n.lineTokens.getBinaryResult(n.ruleStack, n.lineLength), ruleStack: n.ruleStack, stoppedEarly: n.stoppedEarly };
          }
          _tokenize(B, s, o, n) {
            let R;
            if (this._rootId === -1 && (this._rootId = i.RuleFactory.getCompiledRuleId(this._grammar.repository.$self, this, this._grammar.repository), this.getInjections()), s && s !== A.NULL)
              R = !1, s.reset();
            else {
              R = !0;
              const q = this._basicScopeAttributesProvider.getDefaultAttributes(), d = this.themeProvider.getDefaults(), X = F.EncodedTokenAttributes.set(0, q.languageId, q.tokenType, null, d.fontStyle, d.foregroundId, d.backgroundId), v = this.getRule(this._rootId).getName(null, null);
              let b;
              b = v ? w.createRootAndLookUpScopeName(v, X, this) : w.createRoot("unknown", X), s = new A(null, this._rootId, -1, -1, !1, null, b, b);
            }
            B += `
`;
            const P = this.createOnigString(B), y = P.content.length, S = new E(o, B, this._tokenTypeMatchers, this.balancedBracketSelectors), W = k._tokenizeString(this, P, R, 0, s, S, !0, n);
            return G.disposeOnigString(P), { lineLength: y, lineTokens: S, ruleStack: W.stack, stoppedEarly: W.stoppedEarly };
          }
        }
        function h(C, B) {
          return (C = Y.clone(C)).repository = C.repository || {}, C.repository.$self = { $vscodeTextmateLocation: C.$vscodeTextmateLocation, patterns: C.patterns, name: C.scopeName }, C.repository.$base = B || C.repository.$self, C;
        }
        Q.Grammar = M;
        class w {
          constructor(B, s, o) {
            this.parent = B, this.scopePath = s, this.tokenAttributes = o;
          }
          static fromExtension(B, s) {
            let o = B, n = (B == null ? void 0 : B.scopePath) ?? null;
            for (const R of s)
              n = t.ScopeStack.push(n, R.scopeNames), o = new w(o, n, R.encodedTokenAttributes);
            return o;
          }
          static createRoot(B, s) {
            return new w(null, new t.ScopeStack(null, B), s);
          }
          static createRootAndLookUpScopeName(B, s, o) {
            const n = o.getMetadataForScope(B), R = new t.ScopeStack(null, B), P = o.themeProvider.themeMatch(R), y = w.mergeAttributes(s, n, P);
            return new w(null, R, y);
          }
          get scopeName() {
            return this.scopePath.scopeName;
          }
          toString() {
            return this.getScopeNames().join(" ");
          }
          equals(B) {
            return w.equals(this, B);
          }
          static equals(B, s) {
            for (; ; ) {
              if (B === s || !B && !s)
                return !0;
              if (!B || !s || B.scopeName !== s.scopeName || B.tokenAttributes !== s.tokenAttributes)
                return !1;
              B = B.parent, s = s.parent;
            }
          }
          static mergeAttributes(B, s, o) {
            let n = -1, R = 0, P = 0;
            return o !== null && (n = o.fontStyle, R = o.foregroundId, P = o.backgroundId), F.EncodedTokenAttributes.set(B, s.languageId, s.tokenType, null, n, R, P);
          }
          pushAttributed(B, s) {
            if (B === null)
              return this;
            if (B.indexOf(" ") === -1)
              return w._pushAttributed(this, B, s);
            const o = B.split(/ /g);
            let n = this;
            for (const R of o)
              n = w._pushAttributed(n, R, s);
            return n;
          }
          static _pushAttributed(B, s, o) {
            const n = o.getMetadataForScope(s), R = B.scopePath.push(s), P = o.themeProvider.themeMatch(R), y = w.mergeAttributes(B.tokenAttributes, n, P);
            return new w(B, R, y);
          }
          getScopeNames() {
            return this.scopePath.getSegments();
          }
          getExtensionIfDefined(B) {
            var n;
            const s = [];
            let o = this;
            for (; o && o !== B; )
              s.push({ encodedTokenAttributes: o.tokenAttributes, scopeNames: o.scopePath.getExtensionIfDefined(((n = o.parent) == null ? void 0 : n.scopePath) ?? null) }), o = o.parent;
            return o === B ? s.reverse() : void 0;
          }
        }
        Q.AttributedScopeStack = w;
        class A {
          constructor(B, s, o, n, R, P, y, S) {
            this.parent = B, this.ruleId = s, this.beginRuleCapturedEOL = R, this.endRule = P, this.nameScopesList = y, this.contentNameScopesList = S, this._stackElementBrand = void 0, this.depth = this.parent ? this.parent.depth + 1 : 1, this._enterPos = o, this._anchorPos = n;
          }
          equals(B) {
            return B !== null && A._equals(this, B);
          }
          static _equals(B, s) {
            return B === s || !!this._structuralEquals(B, s) && w.equals(B.contentNameScopesList, s.contentNameScopesList);
          }
          static _structuralEquals(B, s) {
            for (; ; ) {
              if (B === s || !B && !s)
                return !0;
              if (!B || !s || B.depth !== s.depth || B.ruleId !== s.ruleId || B.endRule !== s.endRule)
                return !1;
              B = B.parent, s = s.parent;
            }
          }
          clone() {
            return this;
          }
          static _reset(B) {
            for (; B; )
              B._enterPos = -1, B._anchorPos = -1, B = B.parent;
          }
          reset() {
            A._reset(this);
          }
          pop() {
            return this.parent;
          }
          safePop() {
            return this.parent ? this.parent : this;
          }
          push(B, s, o, n, R, P, y) {
            return new A(this, B, s, o, n, R, P, y);
          }
          getEnterPos() {
            return this._enterPos;
          }
          getAnchorPos() {
            return this._anchorPos;
          }
          getRule(B) {
            return B.getRule(this.ruleId);
          }
          toString() {
            const B = [];
            return this._writeString(B, 0), "[" + B.join(",") + "]";
          }
          _writeString(B, s) {
            var o, n;
            return this.parent && (s = this.parent._writeString(B, s)), B[s++] = `(${this.ruleId}, ${(o = this.nameScopesList) == null ? void 0 : o.toString()}, ${(n = this.contentNameScopesList) == null ? void 0 : n.toString()})`, s;
          }
          withContentNameScopesList(B) {
            return this.contentNameScopesList === B ? this : this.parent.push(this.ruleId, this._enterPos, this._anchorPos, this.beginRuleCapturedEOL, this.endRule, this.nameScopesList, B);
          }
          withEndRule(B) {
            return this.endRule === B ? this : new A(this.parent, this.ruleId, this._enterPos, this._anchorPos, this.beginRuleCapturedEOL, B, this.nameScopesList, this.contentNameScopesList);
          }
          hasSameRuleAs(B) {
            let s = this;
            for (; s && s._enterPos === B._enterPos; ) {
              if (s.ruleId === B.ruleId)
                return !0;
              s = s.parent;
            }
            return !1;
          }
          toStateStackFrame() {
            var B, s, o;
            return { ruleId: i.ruleIdToNumber(this.ruleId), beginRuleCapturedEOL: this.beginRuleCapturedEOL, endRule: this.endRule, nameScopesList: ((s = this.nameScopesList) == null ? void 0 : s.getExtensionIfDefined(((B = this.parent) == null ? void 0 : B.nameScopesList) ?? null)) ?? [], contentNameScopesList: ((o = this.contentNameScopesList) == null ? void 0 : o.getExtensionIfDefined(this.nameScopesList)) ?? [] };
          }
          static pushFrame(B, s) {
            const o = w.fromExtension((B == null ? void 0 : B.nameScopesList) ?? null, s.nameScopesList);
            return new A(B, i.ruleIdFromNumber(s.ruleId), s.enterPos ?? -1, s.anchorPos ?? -1, s.beginRuleCapturedEOL, s.endRule, o, w.fromExtension(o, s.contentNameScopesList));
          }
        }
        Q.StateStackImpl = A, A.NULL = new A(null, 0, 0, 0, !1, null, null, null), Q.BalancedBracketSelectors = class {
          constructor(C, B) {
            this.allowAny = !1, this.balancedBracketScopes = C.flatMap((s) => s === "*" ? (this.allowAny = !0, []) : g.createMatchers(s, e).map((o) => o.matcher)), this.unbalancedBracketScopes = B.flatMap((s) => g.createMatchers(s, e).map((o) => o.matcher));
          }
          get matchesAlways() {
            return this.allowAny && this.unbalancedBracketScopes.length === 0;
          }
          get matchesNever() {
            return this.balancedBracketScopes.length === 0 && !this.allowAny;
          }
          match(C) {
            for (const B of this.unbalancedBracketScopes)
              if (B(C))
                return !1;
            for (const B of this.balancedBracketScopes)
              if (B(C))
                return !0;
            return this.allowAny;
          }
        };
        class E {
          constructor(B, s, o, n) {
            this.balancedBracketSelectors = n, this._emitBinaryTokens = B, this._tokenTypeOverrides = o, D.DebugFlags.InDebugMode ? this._lineText = s : this._lineText = null, this._tokens = [], this._binaryTokens = [], this._lastTokenEndIndex = 0;
          }
          produce(B, s) {
            this.produceFromScopes(B.contentNameScopesList, s);
          }
          produceFromScopes(B, s) {
            var n;
            if (this._lastTokenEndIndex >= s)
              return;
            if (this._emitBinaryTokens) {
              let R = (B == null ? void 0 : B.tokenAttributes) ?? 0, P = !1;
              if ((n = this.balancedBracketSelectors) != null && n.matchesAlways && (P = !0), this._tokenTypeOverrides.length > 0 || this.balancedBracketSelectors && !this.balancedBracketSelectors.matchesAlways && !this.balancedBracketSelectors.matchesNever) {
                const y = (B == null ? void 0 : B.getScopeNames()) ?? [];
                for (const S of this._tokenTypeOverrides)
                  S.matcher(y) && (R = F.EncodedTokenAttributes.set(R, 0, F.toOptionalTokenType(S.type), null, -1, 0, 0));
                this.balancedBracketSelectors && (P = this.balancedBracketSelectors.match(y));
              }
              if (P && (R = F.EncodedTokenAttributes.set(R, 0, 8, P, -1, 0, 0)), this._binaryTokens.length > 0 && this._binaryTokens[this._binaryTokens.length - 1] === R)
                return void (this._lastTokenEndIndex = s);
              if (D.DebugFlags.InDebugMode) {
                const y = (B == null ? void 0 : B.getScopeNames()) ?? [];
                console.log("  token: |" + this._lineText.substring(this._lastTokenEndIndex, s).replace(/\n$/, "\\n") + "|");
                for (let S = 0; S < y.length; S++)
                  console.log("      * " + y[S]);
              }
              return this._binaryTokens.push(this._lastTokenEndIndex), this._binaryTokens.push(R), void (this._lastTokenEndIndex = s);
            }
            const o = (B == null ? void 0 : B.getScopeNames()) ?? [];
            if (D.DebugFlags.InDebugMode) {
              console.log("  token: |" + this._lineText.substring(this._lastTokenEndIndex, s).replace(/\n$/, "\\n") + "|");
              for (let R = 0; R < o.length; R++)
                console.log("      * " + o[R]);
            }
            this._tokens.push({ startIndex: this._lastTokenEndIndex, endIndex: s, scopes: o }), this._lastTokenEndIndex = s;
          }
          getResult(B, s) {
            return this._tokens.length > 0 && this._tokens[this._tokens.length - 1].startIndex === s - 1 && this._tokens.pop(), this._tokens.length === 0 && (this._lastTokenEndIndex = -1, this.produce(B, s), this._tokens[this._tokens.length - 1].startIndex = 0), this._tokens;
          }
          getBinaryResult(B, s) {
            this._binaryTokens.length > 0 && this._binaryTokens[this._binaryTokens.length - 2] === s - 1 && (this._binaryTokens.pop(), this._binaryTokens.pop()), this._binaryTokens.length === 0 && (this._lastTokenEndIndex = -1, this.produce(B, s), this._binaryTokens[this._binaryTokens.length - 2] = 0);
            const o = new Uint32Array(this._binaryTokens.length);
            for (let n = 0, R = this._binaryTokens.length; n < R; n++)
              o[n] = this._binaryTokens[n];
            return o;
          }
        }
        Q.LineTokens = E;
      }, 965: (K, Q, N) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.parseInclude = Q.TopLevelRepositoryReference = Q.TopLevelReference = Q.RelativeReference = Q.SelfReference = Q.BaseReference = Q.ScopeDependencyProcessor = Q.ExternalReferenceCollector = Q.TopLevelRepositoryRuleReference = Q.TopLevelRuleReference = void 0;
        const D = N(878);
        class F {
          constructor(A) {
            this.scopeName = A;
          }
          toKey() {
            return this.scopeName;
          }
        }
        Q.TopLevelRuleReference = F;
        class g {
          constructor(A, E) {
            this.scopeName = A, this.ruleName = E;
          }
          toKey() {
            return `${this.scopeName}#${this.ruleName}`;
          }
        }
        Q.TopLevelRepositoryRuleReference = g;
        class G {
          constructor() {
            this._references = [], this._seenReferenceKeys = /* @__PURE__ */ new Set(), this.visitedRule = /* @__PURE__ */ new Set();
          }
          get references() {
            return this._references;
          }
          add(A) {
            const E = A.toKey();
            this._seenReferenceKeys.has(E) || (this._seenReferenceKeys.add(E), this._references.push(A));
          }
        }
        function i(w, A, E, C) {
          const B = E.lookup(w.scopeName);
          if (!B) {
            if (w.scopeName === A)
              throw new Error(`No grammar provided for <${A}>`);
            return;
          }
          const s = E.lookup(A);
          w instanceof F ? Y({ baseGrammar: s, selfGrammar: B }, C) : t(w.ruleName, { baseGrammar: s, selfGrammar: B, repository: B.repository }, C);
          const o = E.injections(w.scopeName);
          if (o)
            for (const n of o)
              C.add(new F(n));
        }
        function t(w, A, E) {
          A.repository && A.repository[w] && a([A.repository[w]], A, E);
        }
        function Y(w, A) {
          w.selfGrammar.patterns && Array.isArray(w.selfGrammar.patterns) && a(w.selfGrammar.patterns, { ...w, repository: w.selfGrammar.repository }, A), w.selfGrammar.injections && a(Object.values(w.selfGrammar.injections), { ...w, repository: w.selfGrammar.repository }, A);
        }
        function a(w, A, E) {
          for (const C of w) {
            if (E.visitedRule.has(C))
              continue;
            E.visitedRule.add(C);
            const B = C.repository ? D.mergeObjects({}, A.repository, C.repository) : A.repository;
            Array.isArray(C.patterns) && a(C.patterns, { ...A, repository: B }, E);
            const s = C.include;
            if (!s)
              continue;
            const o = h(s);
            switch (o.kind) {
              case 0:
                Y({ ...A, selfGrammar: A.baseGrammar }, E);
                break;
              case 1:
                Y(A, E);
                break;
              case 2:
                t(o.ruleName, { ...A, repository: B }, E);
                break;
              case 3:
              case 4:
                const n = o.scopeName === A.selfGrammar.scopeName ? A.selfGrammar : o.scopeName === A.baseGrammar.scopeName ? A.baseGrammar : void 0;
                if (n) {
                  const R = { baseGrammar: A.baseGrammar, selfGrammar: n, repository: B };
                  o.kind === 4 ? t(o.ruleName, R, E) : Y(R, E);
                } else
                  o.kind === 4 ? E.add(new g(o.scopeName, o.ruleName)) : E.add(new F(o.scopeName));
            }
          }
        }
        Q.ExternalReferenceCollector = G, Q.ScopeDependencyProcessor = class {
          constructor(w, A) {
            this.repo = w, this.initialScopeName = A, this.seenFullScopeRequests = /* @__PURE__ */ new Set(), this.seenPartialScopeRequests = /* @__PURE__ */ new Set(), this.seenFullScopeRequests.add(this.initialScopeName), this.Q = [new F(this.initialScopeName)];
          }
          processQueue() {
            const w = this.Q;
            this.Q = [];
            const A = new G();
            for (const E of w)
              i(E, this.initialScopeName, this.repo, A);
            for (const E of A.references)
              if (E instanceof F) {
                if (this.seenFullScopeRequests.has(E.scopeName))
                  continue;
                this.seenFullScopeRequests.add(E.scopeName), this.Q.push(E);
              } else {
                if (this.seenFullScopeRequests.has(E.scopeName) || this.seenPartialScopeRequests.has(E.toKey()))
                  continue;
                this.seenPartialScopeRequests.add(E.toKey()), this.Q.push(E);
              }
          }
        };
        class k {
          constructor() {
            this.kind = 0;
          }
        }
        Q.BaseReference = k;
        class H {
          constructor() {
            this.kind = 1;
          }
        }
        Q.SelfReference = H;
        class e {
          constructor(A) {
            this.ruleName = A, this.kind = 2;
          }
        }
        Q.RelativeReference = e;
        class I {
          constructor(A) {
            this.scopeName = A, this.kind = 3;
          }
        }
        Q.TopLevelReference = I;
        class M {
          constructor(A, E) {
            this.scopeName = A, this.ruleName = E, this.kind = 4;
          }
        }
        function h(w) {
          if (w === "$base")
            return new k();
          if (w === "$self")
            return new H();
          const A = w.indexOf("#");
          if (A === -1)
            return new I(w);
          if (A === 0)
            return new e(w.substring(1));
          {
            const E = w.substring(0, A), C = w.substring(A + 1);
            return new M(E, C);
          }
        }
        Q.TopLevelRepositoryReference = M, Q.parseInclude = h;
      }, 391: function(K, Q, N) {
        var D = this && this.__createBinding || (Object.create ? function(g, G, i, t) {
          t === void 0 && (t = i), Object.defineProperty(g, t, { enumerable: !0, get: function() {
            return G[i];
          } });
        } : function(g, G, i, t) {
          t === void 0 && (t = i), g[t] = G[i];
        }), F = this && this.__exportStar || function(g, G) {
          for (var i in g)
            i === "default" || Object.prototype.hasOwnProperty.call(G, i) || D(G, g, i);
        };
        Object.defineProperty(Q, "__esModule", { value: !0 }), F(N(947), Q);
      }, 47: (K, Q, N) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.LocalStackElement = Q._tokenizeString = void 0;
        const D = N(350), F = N(44), g = N(792), G = N(878);
        class i {
          constructor(M, h) {
            this.stack = M, this.stoppedEarly = h;
          }
        }
        function t(I, M, h, w, A, E, C, B) {
          const s = M.content.length;
          let o = !1, n = -1;
          if (C) {
            const y = function(S, W, q, d, X, v) {
              let b = X.beginRuleCapturedEOL ? 0 : -1;
              const O = [];
              for (let l = X; l; l = l.pop()) {
                const f = l.getRule(S);
                f instanceof g.BeginWhileRule && O.push({ rule: f, stack: l });
              }
              for (let l = O.pop(); l; l = O.pop()) {
                const { ruleScanner: f, findOptions: j } = a(l.rule, S, l.stack.endRule, q, d === b), U = f.findNextMatchSync(W, d, j);
                if (D.DebugFlags.InDebugMode && (console.log("  scanning for while rule"), console.log(f.toString())), !U) {
                  D.DebugFlags.InDebugMode && console.log("  popping " + l.rule.debugName + " - " + l.rule.debugWhileRegExp), X = l.stack.pop();
                  break;
                }
                if (U.ruleId !== g.whileRuleId) {
                  X = l.stack.pop();
                  break;
                }
                U.captureIndices && U.captureIndices.length && (v.produce(l.stack, U.captureIndices[0].start), H(S, W, q, l.stack, v, l.rule.whileCaptures, U.captureIndices), v.produce(l.stack, U.captureIndices[0].end), b = U.captureIndices[0].end, U.captureIndices[0].end > d && (d = U.captureIndices[0].end, q = !1));
              }
              return { stack: X, linePos: d, anchorPosition: b, isFirstLine: q };
            }(I, M, h, w, A, E);
            A = y.stack, w = y.linePos, h = y.isFirstLine, n = y.anchorPosition;
          }
          const R = Date.now();
          for (; !o; ) {
            if (B !== 0 && Date.now() - R > B)
              return new i(A, !0);
            P();
          }
          return new i(A, !1);
          function P() {
            D.DebugFlags.InDebugMode && (console.log(""), console.log(`@@scanNext ${w}: |${M.content.substr(w).replace(/\n$/, "\\n")}|`));
            const y = function(d, X, v, b, O, l) {
              const f = function(BA, DA, GA, gA, EA, eA) {
                const wA = EA.getRule(BA), { ruleScanner: oA, findOptions: sA } = Y(wA, BA, EA.endRule, GA, gA === eA);
                let CA = 0;
                D.DebugFlags.InDebugMode && (CA = G.performanceNow());
                const AA = oA.findNextMatchSync(DA, gA, sA);
                if (D.DebugFlags.InDebugMode) {
                  const iA = G.performanceNow() - CA;
                  iA > 5 && console.warn(`Rule ${wA.debugName} (${wA.id}) matching took ${iA} against '${DA}'`), console.log(`  scanning for (linePos: ${gA}, anchorPosition: ${eA})`), console.log(oA.toString()), AA && console.log(`matched rule id: ${AA.ruleId} from ${AA.captureIndices[0].start} to ${AA.captureIndices[0].end}`);
                }
                return AA ? { captureIndices: AA.captureIndices, matchedRuleId: AA.ruleId } : null;
              }(d, X, v, b, O, l), j = d.getInjections();
              if (j.length === 0)
                return f;
              const U = function(BA, DA, GA, gA, EA, eA, wA) {
                let oA, sA = Number.MAX_VALUE, CA = null, AA = 0;
                const iA = eA.contentNameScopesList.getScopeNames();
                for (let cA = 0, J = BA.length; cA < J; cA++) {
                  const u = BA[cA];
                  if (!u.matcher(iA))
                    continue;
                  const _ = DA.getRule(u.ruleId), { ruleScanner: QA, findOptions: T } = Y(_, DA, null, gA, EA === wA), m = QA.findNextMatchSync(GA, EA, T);
                  if (!m)
                    continue;
                  D.DebugFlags.InDebugMode && (console.log(`  matched injection: ${u.debugSelector}`), console.log(QA.toString()));
                  const $ = m.captureIndices[0].start;
                  if (!($ >= sA) && (sA = $, CA = m.captureIndices, oA = m.ruleId, AA = u.priority, sA === EA))
                    break;
                }
                return CA ? { priorityMatch: AA === -1, captureIndices: CA, matchedRuleId: oA } : null;
              }(j, d, X, v, b, O, l);
              if (!U)
                return f;
              if (!f)
                return U;
              const Z = f.captureIndices[0].start, x = U.captureIndices[0].start;
              return x < Z || U.priorityMatch && x === Z ? U : f;
            }(I, M, h, w, A, n);
            if (!y)
              return D.DebugFlags.InDebugMode && console.log("  no more matches."), E.produce(A, s), void (o = !0);
            const S = y.captureIndices, W = y.matchedRuleId, q = !!(S && S.length > 0) && S[0].end > w;
            if (W === g.endRuleId) {
              const d = A.getRule(I);
              D.DebugFlags.InDebugMode && console.log("  popping " + d.debugName + " - " + d.debugEndRegExp), E.produce(A, S[0].start), A = A.withContentNameScopesList(A.nameScopesList), H(I, M, h, A, E, d.endCaptures, S), E.produce(A, S[0].end);
              const X = A;
              if (A = A.parent, n = X.getAnchorPos(), !q && X.getEnterPos() === w)
                return D.DebugFlags.InDebugMode && console.error("[1] - Grammar is in an endless loop - Grammar pushed & popped a rule without advancing"), A = X, E.produce(A, s), void (o = !0);
            } else {
              const d = I.getRule(W);
              E.produce(A, S[0].start);
              const X = A, v = d.getName(M.content, S), b = A.contentNameScopesList.pushAttributed(v, I);
              if (A = A.push(W, w, n, S[0].end === s, null, b, b), d instanceof g.BeginEndRule) {
                const O = d;
                D.DebugFlags.InDebugMode && console.log("  pushing " + O.debugName + " - " + O.debugBeginRegExp), H(I, M, h, A, E, O.beginCaptures, S), E.produce(A, S[0].end), n = S[0].end;
                const l = O.getContentName(M.content, S), f = b.pushAttributed(l, I);
                if (A = A.withContentNameScopesList(f), O.endHasBackReferences && (A = A.withEndRule(O.getEndWithResolvedBackReferences(M.content, S))), !q && X.hasSameRuleAs(A))
                  return D.DebugFlags.InDebugMode && console.error("[2] - Grammar is in an endless loop - Grammar pushed the same rule without advancing"), A = A.pop(), E.produce(A, s), void (o = !0);
              } else if (d instanceof g.BeginWhileRule) {
                const O = d;
                D.DebugFlags.InDebugMode && console.log("  pushing " + O.debugName), H(I, M, h, A, E, O.beginCaptures, S), E.produce(A, S[0].end), n = S[0].end;
                const l = O.getContentName(M.content, S), f = b.pushAttributed(l, I);
                if (A = A.withContentNameScopesList(f), O.whileHasBackReferences && (A = A.withEndRule(O.getWhileWithResolvedBackReferences(M.content, S))), !q && X.hasSameRuleAs(A))
                  return D.DebugFlags.InDebugMode && console.error("[3] - Grammar is in an endless loop - Grammar pushed the same rule without advancing"), A = A.pop(), E.produce(A, s), void (o = !0);
              } else {
                const O = d;
                if (D.DebugFlags.InDebugMode && console.log("  matched " + O.debugName + " - " + O.debugMatchRegExp), H(I, M, h, A, E, O.captures, S), E.produce(A, S[0].end), A = A.pop(), !q)
                  return D.DebugFlags.InDebugMode && console.error("[4] - Grammar is in an endless loop - Grammar is not advancing, nor is it pushing/popping"), A = A.safePop(), E.produce(A, s), void (o = !0);
              }
            }
            S[0].end > w && (w = S[0].end, h = !1);
          }
        }
        function Y(I, M, h, w, A) {
          return D.UseOnigurumaFindOptions ? { ruleScanner: I.compile(M, h), findOptions: k(w, A) } : { ruleScanner: I.compileAG(M, h, w, A), findOptions: 0 };
        }
        function a(I, M, h, w, A) {
          return D.UseOnigurumaFindOptions ? { ruleScanner: I.compileWhile(M, h), findOptions: k(w, A) } : { ruleScanner: I.compileWhileAG(M, h, w, A), findOptions: 0 };
        }
        function k(I, M) {
          let h = 0;
          return I || (h |= 1), M || (h |= 4), h;
        }
        function H(I, M, h, w, A, E, C) {
          if (E.length === 0)
            return;
          const B = M.content, s = Math.min(E.length, C.length), o = [], n = C[0].end;
          for (let R = 0; R < s; R++) {
            const P = E[R];
            if (P === null)
              continue;
            const y = C[R];
            if (y.length === 0)
              continue;
            if (y.start > n)
              break;
            for (; o.length > 0 && o[o.length - 1].endPos <= y.start; )
              A.produceFromScopes(o[o.length - 1].scopes, o[o.length - 1].endPos), o.pop();
            if (o.length > 0 ? A.produceFromScopes(o[o.length - 1].scopes, y.start) : A.produce(w, y.start), P.retokenizeCapturedWithRuleId) {
              const W = P.getName(B, C), q = w.contentNameScopesList.pushAttributed(W, I), d = P.getContentName(B, C), X = q.pushAttributed(d, I), v = w.push(P.retokenizeCapturedWithRuleId, y.start, -1, !1, null, q, X), b = I.createOnigString(B.substring(0, y.end));
              t(I, b, h && y.start === 0, y.start, v, A, !1, 0), F.disposeOnigString(b);
              continue;
            }
            const S = P.getName(B, C);
            if (S !== null) {
              const W = (o.length > 0 ? o[o.length - 1].scopes : w.contentNameScopesList).pushAttributed(S, I);
              o.push(new e(W, y.end));
            }
          }
          for (; o.length > 0; )
            A.produceFromScopes(o[o.length - 1].scopes, o[o.length - 1].endPos), o.pop();
        }
        Q._tokenizeString = t;
        class e {
          constructor(M, h) {
            this.scopes = M, this.endPos = h;
          }
        }
        Q.LocalStackElement = e;
      }, 974: (K, Q) => {
        function N(G, i) {
          throw new Error("Near offset " + G.pos + ": " + i + " ~~~" + G.source.substr(G.pos, 50) + "~~~");
        }
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.parseJSON = void 0, Q.parseJSON = function(G, i, t) {
          let Y = new D(G), a = new F(), k = 0, H = null, e = [], I = [];
          function M() {
            e.push(k), I.push(H);
          }
          function h() {
            k = e.pop(), H = I.pop();
          }
          function w(A) {
            N(Y, A);
          }
          for (; g(Y, a); ) {
            if (k === 0) {
              if (H !== null && w("too many constructs in root"), a.type === 3) {
                H = {}, t && (H.$vscodeTextmateLocation = a.toLocation(i)), M(), k = 1;
                continue;
              }
              if (a.type === 2) {
                H = [], M(), k = 4;
                continue;
              }
              w("unexpected token in root");
            }
            if (k === 2) {
              if (a.type === 5) {
                h();
                continue;
              }
              if (a.type === 7) {
                k = 3;
                continue;
              }
              w("expected , or }");
            }
            if (k === 1 || k === 3) {
              if (k === 1 && a.type === 5) {
                h();
                continue;
              }
              if (a.type === 1) {
                let A = a.value;
                if (g(Y, a) && a.type === 6 || w("expected colon"), g(Y, a) || w("expected value"), k = 2, a.type === 1) {
                  H[A] = a.value;
                  continue;
                }
                if (a.type === 8) {
                  H[A] = null;
                  continue;
                }
                if (a.type === 9) {
                  H[A] = !0;
                  continue;
                }
                if (a.type === 10) {
                  H[A] = !1;
                  continue;
                }
                if (a.type === 11) {
                  H[A] = parseFloat(a.value);
                  continue;
                }
                if (a.type === 2) {
                  let E = [];
                  H[A] = E, M(), k = 4, H = E;
                  continue;
                }
                if (a.type === 3) {
                  let E = {};
                  t && (E.$vscodeTextmateLocation = a.toLocation(i)), H[A] = E, M(), k = 1, H = E;
                  continue;
                }
              }
              w("unexpected token in dict");
            }
            if (k === 5) {
              if (a.type === 4) {
                h();
                continue;
              }
              if (a.type === 7) {
                k = 6;
                continue;
              }
              w("expected , or ]");
            }
            if (k === 4 || k === 6) {
              if (k === 4 && a.type === 4) {
                h();
                continue;
              }
              if (k = 5, a.type === 1) {
                H.push(a.value);
                continue;
              }
              if (a.type === 8) {
                H.push(null);
                continue;
              }
              if (a.type === 9) {
                H.push(!0);
                continue;
              }
              if (a.type === 10) {
                H.push(!1);
                continue;
              }
              if (a.type === 11) {
                H.push(parseFloat(a.value));
                continue;
              }
              if (a.type === 2) {
                let A = [];
                H.push(A), M(), k = 4, H = A;
                continue;
              }
              if (a.type === 3) {
                let A = {};
                t && (A.$vscodeTextmateLocation = a.toLocation(i)), H.push(A), M(), k = 1, H = A;
                continue;
              }
              w("unexpected token in array");
            }
            w("unknown state");
          }
          return I.length !== 0 && w("unclosed constructs"), H;
        };
        class D {
          constructor(i) {
            this.source = i, this.pos = 0, this.len = i.length, this.line = 1, this.char = 0;
          }
        }
        class F {
          constructor() {
            this.value = null, this.type = 0, this.offset = -1, this.len = -1, this.line = -1, this.char = -1;
          }
          toLocation(i) {
            return { filename: i, line: this.line, char: this.char };
          }
        }
        function g(G, i) {
          i.value = null, i.type = 0, i.offset = -1, i.len = -1, i.line = -1, i.char = -1;
          let t, Y = G.source, a = G.pos, k = G.len, H = G.line, e = G.char;
          for (; ; ) {
            if (a >= k)
              return !1;
            if (t = Y.charCodeAt(a), t !== 32 && t !== 9 && t !== 13) {
              if (t !== 10)
                break;
              a++, H++, e = 0;
            } else
              a++, e++;
          }
          if (i.offset = a, i.line = H, i.char = e, t === 34) {
            for (i.type = 1, a++, e++; ; ) {
              if (a >= k)
                return !1;
              if (t = Y.charCodeAt(a), a++, e++, t !== 92) {
                if (t === 34)
                  break;
              } else
                a++, e++;
            }
            i.value = Y.substring(i.offset + 1, a - 1).replace(/\\u([0-9A-Fa-f]{4})/g, (I, M) => String.fromCodePoint(parseInt(M, 16))).replace(/\\(.)/g, (I, M) => {
              switch (M) {
                case '"':
                  return '"';
                case "\\":
                  return "\\";
                case "/":
                  return "/";
                case "b":
                  return "\b";
                case "f":
                  return "\f";
                case "n":
                  return `
`;
                case "r":
                  return "\r";
                case "t":
                  return "	";
                default:
                  N(G, "invalid escape sequence");
              }
              throw new Error("unreachable");
            });
          } else if (t === 91)
            i.type = 2, a++, e++;
          else if (t === 123)
            i.type = 3, a++, e++;
          else if (t === 93)
            i.type = 4, a++, e++;
          else if (t === 125)
            i.type = 5, a++, e++;
          else if (t === 58)
            i.type = 6, a++, e++;
          else if (t === 44)
            i.type = 7, a++, e++;
          else if (t === 110) {
            if (i.type = 8, a++, e++, t = Y.charCodeAt(a), t !== 117 || (a++, e++, t = Y.charCodeAt(a), t !== 108) || (a++, e++, t = Y.charCodeAt(a), t !== 108))
              return !1;
            a++, e++;
          } else if (t === 116) {
            if (i.type = 9, a++, e++, t = Y.charCodeAt(a), t !== 114 || (a++, e++, t = Y.charCodeAt(a), t !== 117) || (a++, e++, t = Y.charCodeAt(a), t !== 101))
              return !1;
            a++, e++;
          } else if (t === 102) {
            if (i.type = 10, a++, e++, t = Y.charCodeAt(a), t !== 97 || (a++, e++, t = Y.charCodeAt(a), t !== 108) || (a++, e++, t = Y.charCodeAt(a), t !== 115) || (a++, e++, t = Y.charCodeAt(a), t !== 101))
              return !1;
            a++, e++;
          } else
            for (i.type = 11; ; ) {
              if (a >= k)
                return !1;
              if (t = Y.charCodeAt(a), !(t === 46 || t >= 48 && t <= 57 || t === 101 || t === 69 || t === 45 || t === 43))
                break;
              a++, e++;
            }
          return i.len = a - i.offset, i.value === null && (i.value = Y.substr(i.offset, i.len)), G.pos = a, G.line = H, G.char = e, !0;
        }
      }, 787: function(K, Q, N) {
        var D = this && this.__createBinding || (Object.create ? function(k, H, e, I) {
          I === void 0 && (I = e), Object.defineProperty(k, I, { enumerable: !0, get: function() {
            return H[e];
          } });
        } : function(k, H, e, I) {
          I === void 0 && (I = e), k[I] = H[e];
        }), F = this && this.__exportStar || function(k, H) {
          for (var e in k)
            e === "default" || Object.prototype.hasOwnProperty.call(H, e) || D(H, k, e);
        };
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.applyStateStackDiff = Q.diffStateStacksRefEq = Q.parseRawGrammar = Q.INITIAL = Q.Registry = void 0;
        const g = N(391), G = N(50), i = N(652), t = N(583), Y = N(965), a = N(442);
        Object.defineProperty(Q, "applyStateStackDiff", { enumerable: !0, get: function() {
          return a.applyStateStackDiff;
        } }), Object.defineProperty(Q, "diffStateStacksRefEq", { enumerable: !0, get: function() {
          return a.diffStateStacksRefEq;
        } }), F(N(44), Q), Q.Registry = class {
          constructor(k) {
            this._options = k, this._syncRegistry = new i.SyncRegistry(t.Theme.createFromRawTheme(k.theme, k.colorMap), k.onigLib), this._ensureGrammarCache = /* @__PURE__ */ new Map();
          }
          dispose() {
            this._syncRegistry.dispose();
          }
          setTheme(k, H) {
            this._syncRegistry.setTheme(t.Theme.createFromRawTheme(k, H));
          }
          getColorMap() {
            return this._syncRegistry.getColorMap();
          }
          loadGrammarWithEmbeddedLanguages(k, H, e) {
            return this.loadGrammarWithConfiguration(k, H, { embeddedLanguages: e });
          }
          loadGrammarWithConfiguration(k, H, e) {
            return this._loadGrammar(k, H, e.embeddedLanguages, e.tokenTypes, new g.BalancedBracketSelectors(e.balancedBracketSelectors || [], e.unbalancedBracketSelectors || []));
          }
          loadGrammar(k) {
            return this._loadGrammar(k, 0, null, null, null);
          }
          async _loadGrammar(k, H, e, I, M) {
            const h = new Y.ScopeDependencyProcessor(this._syncRegistry, k);
            for (; h.Q.length > 0; )
              await Promise.all(h.Q.map((w) => this._loadSingleGrammar(w.scopeName))), h.processQueue();
            return this._grammarForScopeName(k, H, e, I, M);
          }
          async _loadSingleGrammar(k) {
            return this._ensureGrammarCache.has(k) || this._ensureGrammarCache.set(k, this._doLoadSingleGrammar(k)), this._ensureGrammarCache.get(k);
          }
          async _doLoadSingleGrammar(k) {
            const H = await this._options.loadGrammar(k);
            if (H) {
              const e = typeof this._options.getInjections == "function" ? this._options.getInjections(k) : void 0;
              this._syncRegistry.addGrammar(H, e);
            }
          }
          async addGrammar(k, H = [], e = 0, I = null) {
            return this._syncRegistry.addGrammar(k, H), await this._grammarForScopeName(k.scopeName, e, I);
          }
          _grammarForScopeName(k, H = 0, e = null, I = null, M = null) {
            return this._syncRegistry.grammarForScopeName(k, H, e, I, M);
          }
        }, Q.INITIAL = g.StateStackImpl.NULL, Q.parseRawGrammar = G.parseRawGrammar;
      }, 736: (K, Q) => {
        function N(D) {
          return !!D && !!D.match(/[\w\.:]+/);
        }
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.createMatchers = void 0, Q.createMatchers = function(D, F) {
          const g = [], G = function(a) {
            let k = /([LR]:|[\w\.:][\w\.:\-]*|[\,\|\-\(\)])/g, H = k.exec(a);
            return { next: () => {
              if (!H)
                return null;
              const e = H[0];
              return H = k.exec(a), e;
            } };
          }(D);
          let i = G.next();
          for (; i !== null; ) {
            let a = 0;
            if (i.length === 2 && i.charAt(1) === ":") {
              switch (i.charAt(0)) {
                case "R":
                  a = 1;
                  break;
                case "L":
                  a = -1;
                  break;
                default:
                  console.log(`Unknown priority ${i} in scope selector`);
              }
              i = G.next();
            }
            let k = Y();
            if (g.push({ matcher: k, priority: a }), i !== ",")
              break;
            i = G.next();
          }
          return g;
          function t() {
            if (i === "-") {
              i = G.next();
              const a = t();
              return (k) => !!a && !a(k);
            }
            if (i === "(") {
              i = G.next();
              const a = function() {
                const k = [];
                let H = Y();
                for (; H && (k.push(H), i === "|" || i === ","); ) {
                  do
                    i = G.next();
                  while (i === "|" || i === ",");
                  H = Y();
                }
                return (e) => k.some((I) => I(e));
              }();
              return i === ")" && (i = G.next()), a;
            }
            if (N(i)) {
              const a = [];
              do
                a.push(i), i = G.next();
              while (N(i));
              return (k) => F(a, k);
            }
            return null;
          }
          function Y() {
            const a = [];
            let k = t();
            for (; k; )
              a.push(k), k = t();
            return (H) => a.every((e) => e(H));
          }
        };
      }, 44: (K, Q) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.disposeOnigString = void 0, Q.disposeOnigString = function(N) {
          typeof N.dispose == "function" && N.dispose();
        };
      }, 50: (K, Q, N) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.parseRawGrammar = void 0;
        const D = N(69), F = N(350), g = N(974);
        Q.parseRawGrammar = function(G, i = null) {
          return i !== null && /\.json$/.test(i) ? (t = G, Y = i, F.DebugFlags.InDebugMode ? g.parseJSON(t, Y, !0) : JSON.parse(t)) : function(a, k) {
            return F.DebugFlags.InDebugMode ? D.parseWithLocation(a, k, "$vscodeTextmateLocation") : D.parsePLIST(a);
          }(G, i);
          var t, Y;
        };
      }, 69: (K, Q) => {
        function N(D, F, g) {
          const G = D.length;
          let i = 0, t = 1, Y = 0;
          function a(U) {
            if (g === null)
              i += U;
            else
              for (; U > 0; )
                D.charCodeAt(i) === 10 ? (i++, t++, Y = 0) : (i++, Y++), U--;
          }
          function k(U) {
            g === null ? i = U : a(U - i);
          }
          function H() {
            for (; i < G; ) {
              let U = D.charCodeAt(i);
              if (U !== 32 && U !== 9 && U !== 13 && U !== 10)
                break;
              a(1);
            }
          }
          function e(U) {
            return D.substr(i, U.length) === U && (a(U.length), !0);
          }
          function I(U) {
            let Z = D.indexOf(U, i);
            k(Z !== -1 ? Z + U.length : G);
          }
          function M(U) {
            let Z = D.indexOf(U, i);
            if (Z !== -1) {
              let x = D.substring(i, Z);
              return k(Z + U.length), x;
            }
            {
              let x = D.substr(i);
              return k(G), x;
            }
          }
          G > 0 && D.charCodeAt(0) === 65279 && (i = 1);
          let h = 0, w = null, A = [], E = [], C = null;
          function B(U, Z) {
            A.push(h), E.push(w), h = U, w = Z;
          }
          function s() {
            if (A.length === 0)
              return o("illegal state stack");
            h = A.pop(), w = E.pop();
          }
          function o(U) {
            throw new Error("Near offset " + i + ": " + U + " ~~~" + D.substr(i, 50) + "~~~");
          }
          const n = function() {
            if (C === null)
              return o("missing <key>");
            let U = {};
            g !== null && (U[g] = { filename: F, line: t, char: Y }), w[C] = U, C = null, B(1, U);
          }, R = function() {
            if (C === null)
              return o("missing <key>");
            let U = [];
            w[C] = U, C = null, B(2, U);
          }, P = function() {
            let U = {};
            g !== null && (U[g] = { filename: F, line: t, char: Y }), w.push(U), B(1, U);
          }, y = function() {
            let U = [];
            w.push(U), B(2, U);
          };
          function S() {
            if (h !== 1)
              return o("unexpected </dict>");
            s();
          }
          function W() {
            return h === 1 || h !== 2 ? o("unexpected </array>") : void s();
          }
          function q(U) {
            if (h === 1) {
              if (C === null)
                return o("missing <key>");
              w[C] = U, C = null;
            } else
              h === 2 ? w.push(U) : w = U;
          }
          function d(U) {
            if (isNaN(U))
              return o("cannot parse float");
            if (h === 1) {
              if (C === null)
                return o("missing <key>");
              w[C] = U, C = null;
            } else
              h === 2 ? w.push(U) : w = U;
          }
          function X(U) {
            if (isNaN(U))
              return o("cannot parse integer");
            if (h === 1) {
              if (C === null)
                return o("missing <key>");
              w[C] = U, C = null;
            } else
              h === 2 ? w.push(U) : w = U;
          }
          function v(U) {
            if (h === 1) {
              if (C === null)
                return o("missing <key>");
              w[C] = U, C = null;
            } else
              h === 2 ? w.push(U) : w = U;
          }
          function b(U) {
            if (h === 1) {
              if (C === null)
                return o("missing <key>");
              w[C] = U, C = null;
            } else
              h === 2 ? w.push(U) : w = U;
          }
          function O(U) {
            if (h === 1) {
              if (C === null)
                return o("missing <key>");
              w[C] = U, C = null;
            } else
              h === 2 ? w.push(U) : w = U;
          }
          function l() {
            let U = M(">"), Z = !1;
            return U.charCodeAt(U.length - 1) === 47 && (Z = !0, U = U.substring(0, U.length - 1)), { name: U.trim(), isClosed: Z };
          }
          function f(U) {
            if (U.isClosed)
              return "";
            let Z = M("</");
            return I(">"), Z.replace(/&#([0-9]+);/g, function(x, BA) {
              return String.fromCodePoint(parseInt(BA, 10));
            }).replace(/&#x([0-9a-f]+);/g, function(x, BA) {
              return String.fromCodePoint(parseInt(BA, 16));
            }).replace(/&amp;|&lt;|&gt;|&quot;|&apos;/g, function(x) {
              switch (x) {
                case "&amp;":
                  return "&";
                case "&lt;":
                  return "<";
                case "&gt;":
                  return ">";
                case "&quot;":
                  return '"';
                case "&apos;":
                  return "'";
              }
              return x;
            });
          }
          for (; i < G && (H(), !(i >= G)); ) {
            const U = D.charCodeAt(i);
            if (a(1), U !== 60)
              return o("expected <");
            if (i >= G)
              return o("unexpected end of input");
            const Z = D.charCodeAt(i);
            if (Z === 63) {
              a(1), I("?>");
              continue;
            }
            if (Z === 33) {
              if (a(1), e("--")) {
                I("-->");
                continue;
              }
              I(">");
              continue;
            }
            if (Z === 47) {
              if (a(1), H(), e("plist")) {
                I(">");
                continue;
              }
              if (e("dict")) {
                I(">"), S();
                continue;
              }
              if (e("array")) {
                I(">"), W();
                continue;
              }
              return o("unexpected closed tag");
            }
            let x = l();
            switch (x.name) {
              case "dict":
                h === 1 ? n() : h === 2 ? P() : (w = {}, g !== null && (w[g] = { filename: F, line: t, char: Y }), B(1, w)), x.isClosed && S();
                continue;
              case "array":
                h === 1 ? R() : h === 2 ? y() : (w = [], B(2, w)), x.isClosed && W();
                continue;
              case "key":
                j = f(x), h !== 1 ? o("unexpected <key>") : C !== null ? o("too many <key>") : C = j;
                continue;
              case "string":
                q(f(x));
                continue;
              case "real":
                d(parseFloat(f(x)));
                continue;
              case "integer":
                X(parseInt(f(x), 10));
                continue;
              case "date":
                v(new Date(f(x)));
                continue;
              case "data":
                b(f(x));
                continue;
              case "true":
                f(x), O(!0);
                continue;
              case "false":
                f(x), O(!1);
                continue;
            }
            if (!/^plist/.test(x.name))
              return o("unexpected opened tag " + x.name);
          }
          var j;
          return w;
        }
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.parsePLIST = Q.parseWithLocation = void 0, Q.parseWithLocation = function(D, F, g) {
          return N(D, F, g);
        }, Q.parsePLIST = function(D) {
          return N(D, null, null);
        };
      }, 652: (K, Q, N) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.SyncRegistry = void 0;
        const D = N(391);
        Q.SyncRegistry = class {
          constructor(F, g) {
            this._onigLibPromise = g, this._grammars = /* @__PURE__ */ new Map(), this._rawGrammars = /* @__PURE__ */ new Map(), this._injectionGrammars = /* @__PURE__ */ new Map(), this._theme = F;
          }
          dispose() {
            for (const F of this._grammars.values())
              F.dispose();
          }
          setTheme(F) {
            this._theme = F;
          }
          getColorMap() {
            return this._theme.getColorMap();
          }
          addGrammar(F, g) {
            this._rawGrammars.set(F.scopeName, F), g && this._injectionGrammars.set(F.scopeName, g);
          }
          lookup(F) {
            return this._rawGrammars.get(F);
          }
          injections(F) {
            return this._injectionGrammars.get(F);
          }
          getDefaults() {
            return this._theme.getDefaults();
          }
          themeMatch(F) {
            return this._theme.match(F);
          }
          async grammarForScopeName(F, g, G, i, t) {
            if (!this._grammars.has(F)) {
              let Y = this._rawGrammars.get(F);
              if (!Y)
                return null;
              this._grammars.set(F, D.createGrammar(F, Y, g, G, i, t, this, await this._onigLibPromise));
            }
            return this._grammars.get(F);
          }
        };
      }, 792: (K, Q, N) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.CompiledRule = Q.RegExpSourceList = Q.RegExpSource = Q.RuleFactory = Q.BeginWhileRule = Q.BeginEndRule = Q.IncludeOnlyRule = Q.MatchRule = Q.CaptureRule = Q.Rule = Q.ruleIdToNumber = Q.ruleIdFromNumber = Q.whileRuleId = Q.endRuleId = void 0;
        const D = N(878), F = N(965), g = /\\(\d+)/, G = /\\(\d+)/g;
        Q.endRuleId = -1, Q.whileRuleId = -2, Q.ruleIdFromNumber = function(w) {
          return w;
        }, Q.ruleIdToNumber = function(w) {
          return w;
        };
        class i {
          constructor(A, E, C, B) {
            this.$location = A, this.id = E, this._name = C || null, this._nameIsCapturing = D.RegexSource.hasCaptures(this._name), this._contentName = B || null, this._contentNameIsCapturing = D.RegexSource.hasCaptures(this._contentName);
          }
          get debugName() {
            const A = this.$location ? `${D.basename(this.$location.filename)}:${this.$location.line}` : "unknown";
            return `${this.constructor.name}#${this.id} @ ${A}`;
          }
          getName(A, E) {
            return this._nameIsCapturing && this._name !== null && A !== null && E !== null ? D.RegexSource.replaceCaptures(this._name, A, E) : this._name;
          }
          getContentName(A, E) {
            return this._contentNameIsCapturing && this._contentName !== null ? D.RegexSource.replaceCaptures(this._contentName, A, E) : this._contentName;
          }
        }
        Q.Rule = i;
        class t extends i {
          constructor(A, E, C, B, s) {
            super(A, E, C, B), this.retokenizeCapturedWithRuleId = s;
          }
          dispose() {
          }
          collectPatterns(A, E) {
            throw new Error("Not supported!");
          }
          compile(A, E) {
            throw new Error("Not supported!");
          }
          compileAG(A, E, C, B) {
            throw new Error("Not supported!");
          }
        }
        Q.CaptureRule = t;
        class Y extends i {
          constructor(A, E, C, B, s) {
            super(A, E, C, null), this._match = new I(B, this.id), this.captures = s, this._cachedCompiledPatterns = null;
          }
          dispose() {
            this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null);
          }
          get debugMatchRegExp() {
            return `${this._match.source}`;
          }
          collectPatterns(A, E) {
            E.push(this._match);
          }
          compile(A, E) {
            return this._getCachedCompiledPatterns(A).compile(A);
          }
          compileAG(A, E, C, B) {
            return this._getCachedCompiledPatterns(A).compileAG(A, C, B);
          }
          _getCachedCompiledPatterns(A) {
            return this._cachedCompiledPatterns || (this._cachedCompiledPatterns = new M(), this.collectPatterns(A, this._cachedCompiledPatterns)), this._cachedCompiledPatterns;
          }
        }
        Q.MatchRule = Y;
        class a extends i {
          constructor(A, E, C, B, s) {
            super(A, E, C, B), this.patterns = s.patterns, this.hasMissingPatterns = s.hasMissingPatterns, this._cachedCompiledPatterns = null;
          }
          dispose() {
            this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null);
          }
          collectPatterns(A, E) {
            for (const C of this.patterns)
              A.getRule(C).collectPatterns(A, E);
          }
          compile(A, E) {
            return this._getCachedCompiledPatterns(A).compile(A);
          }
          compileAG(A, E, C, B) {
            return this._getCachedCompiledPatterns(A).compileAG(A, C, B);
          }
          _getCachedCompiledPatterns(A) {
            return this._cachedCompiledPatterns || (this._cachedCompiledPatterns = new M(), this.collectPatterns(A, this._cachedCompiledPatterns)), this._cachedCompiledPatterns;
          }
        }
        Q.IncludeOnlyRule = a;
        class k extends i {
          constructor(A, E, C, B, s, o, n, R, P, y) {
            super(A, E, C, B), this._begin = new I(s, this.id), this.beginCaptures = o, this._end = new I(n || "￿", -1), this.endHasBackReferences = this._end.hasBackReferences, this.endCaptures = R, this.applyEndPatternLast = P || !1, this.patterns = y.patterns, this.hasMissingPatterns = y.hasMissingPatterns, this._cachedCompiledPatterns = null;
          }
          dispose() {
            this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null);
          }
          get debugBeginRegExp() {
            return `${this._begin.source}`;
          }
          get debugEndRegExp() {
            return `${this._end.source}`;
          }
          getEndWithResolvedBackReferences(A, E) {
            return this._end.resolveBackReferences(A, E);
          }
          collectPatterns(A, E) {
            E.push(this._begin);
          }
          compile(A, E) {
            return this._getCachedCompiledPatterns(A, E).compile(A);
          }
          compileAG(A, E, C, B) {
            return this._getCachedCompiledPatterns(A, E).compileAG(A, C, B);
          }
          _getCachedCompiledPatterns(A, E) {
            if (!this._cachedCompiledPatterns) {
              this._cachedCompiledPatterns = new M();
              for (const C of this.patterns)
                A.getRule(C).collectPatterns(A, this._cachedCompiledPatterns);
              this.applyEndPatternLast ? this._cachedCompiledPatterns.push(this._end.hasBackReferences ? this._end.clone() : this._end) : this._cachedCompiledPatterns.unshift(this._end.hasBackReferences ? this._end.clone() : this._end);
            }
            return this._end.hasBackReferences && (this.applyEndPatternLast ? this._cachedCompiledPatterns.setSource(this._cachedCompiledPatterns.length() - 1, E) : this._cachedCompiledPatterns.setSource(0, E)), this._cachedCompiledPatterns;
          }
        }
        Q.BeginEndRule = k;
        class H extends i {
          constructor(A, E, C, B, s, o, n, R, P) {
            super(A, E, C, B), this._begin = new I(s, this.id), this.beginCaptures = o, this.whileCaptures = R, this._while = new I(n, Q.whileRuleId), this.whileHasBackReferences = this._while.hasBackReferences, this.patterns = P.patterns, this.hasMissingPatterns = P.hasMissingPatterns, this._cachedCompiledPatterns = null, this._cachedCompiledWhilePatterns = null;
          }
          dispose() {
            this._cachedCompiledPatterns && (this._cachedCompiledPatterns.dispose(), this._cachedCompiledPatterns = null), this._cachedCompiledWhilePatterns && (this._cachedCompiledWhilePatterns.dispose(), this._cachedCompiledWhilePatterns = null);
          }
          get debugBeginRegExp() {
            return `${this._begin.source}`;
          }
          get debugWhileRegExp() {
            return `${this._while.source}`;
          }
          getWhileWithResolvedBackReferences(A, E) {
            return this._while.resolveBackReferences(A, E);
          }
          collectPatterns(A, E) {
            E.push(this._begin);
          }
          compile(A, E) {
            return this._getCachedCompiledPatterns(A).compile(A);
          }
          compileAG(A, E, C, B) {
            return this._getCachedCompiledPatterns(A).compileAG(A, C, B);
          }
          _getCachedCompiledPatterns(A) {
            if (!this._cachedCompiledPatterns) {
              this._cachedCompiledPatterns = new M();
              for (const E of this.patterns)
                A.getRule(E).collectPatterns(A, this._cachedCompiledPatterns);
            }
            return this._cachedCompiledPatterns;
          }
          compileWhile(A, E) {
            return this._getCachedCompiledWhilePatterns(A, E).compile(A);
          }
          compileWhileAG(A, E, C, B) {
            return this._getCachedCompiledWhilePatterns(A, E).compileAG(A, C, B);
          }
          _getCachedCompiledWhilePatterns(A, E) {
            return this._cachedCompiledWhilePatterns || (this._cachedCompiledWhilePatterns = new M(), this._cachedCompiledWhilePatterns.push(this._while.hasBackReferences ? this._while.clone() : this._while)), this._while.hasBackReferences && this._cachedCompiledWhilePatterns.setSource(0, E || "￿"), this._cachedCompiledWhilePatterns;
          }
        }
        Q.BeginWhileRule = H;
        class e {
          static createCaptureRule(A, E, C, B, s) {
            return A.registerRule((o) => new t(E, o, C, B, s));
          }
          static getCompiledRuleId(A, E, C) {
            return A.id || E.registerRule((B) => {
              if (A.id = B, A.match)
                return new Y(A.$vscodeTextmateLocation, A.id, A.name, A.match, e._compileCaptures(A.captures, E, C));
              if (A.begin === void 0) {
                A.repository && (C = D.mergeObjects({}, C, A.repository));
                let s = A.patterns;
                return s === void 0 && A.include && (s = [{ include: A.include }]), new a(A.$vscodeTextmateLocation, A.id, A.name, A.contentName, e._compilePatterns(s, E, C));
              }
              return A.while ? new H(A.$vscodeTextmateLocation, A.id, A.name, A.contentName, A.begin, e._compileCaptures(A.beginCaptures || A.captures, E, C), A.while, e._compileCaptures(A.whileCaptures || A.captures, E, C), e._compilePatterns(A.patterns, E, C)) : new k(A.$vscodeTextmateLocation, A.id, A.name, A.contentName, A.begin, e._compileCaptures(A.beginCaptures || A.captures, E, C), A.end, e._compileCaptures(A.endCaptures || A.captures, E, C), A.applyEndPatternLast, e._compilePatterns(A.patterns, E, C));
            }), A.id;
          }
          static _compileCaptures(A, E, C) {
            let B = [];
            if (A) {
              let s = 0;
              for (const o in A) {
                if (o === "$vscodeTextmateLocation")
                  continue;
                const n = parseInt(o, 10);
                n > s && (s = n);
              }
              for (let o = 0; o <= s; o++)
                B[o] = null;
              for (const o in A) {
                if (o === "$vscodeTextmateLocation")
                  continue;
                const n = parseInt(o, 10);
                let R = 0;
                A[o].patterns && (R = e.getCompiledRuleId(A[o], E, C)), B[n] = e.createCaptureRule(E, A[o].$vscodeTextmateLocation, A[o].name, A[o].contentName, R);
              }
            }
            return B;
          }
          static _compilePatterns(A, E, C) {
            let B = [];
            if (A)
              for (let s = 0, o = A.length; s < o; s++) {
                const n = A[s];
                let R = -1;
                if (n.include) {
                  const P = F.parseInclude(n.include);
                  switch (P.kind) {
                    case 0:
                    case 1:
                      R = e.getCompiledRuleId(C[n.include], E, C);
                      break;
                    case 2:
                      let y = C[P.ruleName];
                      y && (R = e.getCompiledRuleId(y, E, C));
                      break;
                    case 3:
                    case 4:
                      const S = P.scopeName, W = P.kind === 4 ? P.ruleName : null, q = E.getExternalGrammar(S, C);
                      if (q)
                        if (W) {
                          let d = q.repository[W];
                          d && (R = e.getCompiledRuleId(d, E, q.repository));
                        } else
                          R = e.getCompiledRuleId(q.repository.$self, E, q.repository);
                  }
                } else
                  R = e.getCompiledRuleId(n, E, C);
                if (R !== -1) {
                  const P = E.getRule(R);
                  let y = !1;
                  if ((P instanceof a || P instanceof k || P instanceof H) && P.hasMissingPatterns && P.patterns.length === 0 && (y = !0), y)
                    continue;
                  B.push(R);
                }
              }
            return { patterns: B, hasMissingPatterns: (A ? A.length : 0) !== B.length };
          }
        }
        Q.RuleFactory = e;
        class I {
          constructor(A, E) {
            if (A) {
              const C = A.length;
              let B = 0, s = [], o = !1;
              for (let n = 0; n < C; n++)
                if (A.charAt(n) === "\\" && n + 1 < C) {
                  const R = A.charAt(n + 1);
                  R === "z" ? (s.push(A.substring(B, n)), s.push("$(?!\\n)(?<!\\n)"), B = n + 2) : R !== "A" && R !== "G" || (o = !0), n++;
                }
              this.hasAnchor = o, B === 0 ? this.source = A : (s.push(A.substring(B, C)), this.source = s.join(""));
            } else
              this.hasAnchor = !1, this.source = A;
            this.hasAnchor ? this._anchorCache = this._buildAnchorCache() : this._anchorCache = null, this.ruleId = E, this.hasBackReferences = g.test(this.source);
          }
          clone() {
            return new I(this.source, this.ruleId);
          }
          setSource(A) {
            this.source !== A && (this.source = A, this.hasAnchor && (this._anchorCache = this._buildAnchorCache()));
          }
          resolveBackReferences(A, E) {
            let C = E.map((B) => A.substring(B.start, B.end));
            return G.lastIndex = 0, this.source.replace(G, (B, s) => D.escapeRegExpCharacters(C[parseInt(s, 10)] || ""));
          }
          _buildAnchorCache() {
            let A, E, C, B, s = [], o = [], n = [], R = [];
            for (A = 0, E = this.source.length; A < E; A++)
              C = this.source.charAt(A), s[A] = C, o[A] = C, n[A] = C, R[A] = C, C === "\\" && A + 1 < E && (B = this.source.charAt(A + 1), B === "A" ? (s[A + 1] = "￿", o[A + 1] = "￿", n[A + 1] = "A", R[A + 1] = "A") : B === "G" ? (s[A + 1] = "￿", o[A + 1] = "G", n[A + 1] = "￿", R[A + 1] = "G") : (s[A + 1] = B, o[A + 1] = B, n[A + 1] = B, R[A + 1] = B), A++);
            return { A0_G0: s.join(""), A0_G1: o.join(""), A1_G0: n.join(""), A1_G1: R.join("") };
          }
          resolveAnchors(A, E) {
            return this.hasAnchor && this._anchorCache ? A ? E ? this._anchorCache.A1_G1 : this._anchorCache.A1_G0 : E ? this._anchorCache.A0_G1 : this._anchorCache.A0_G0 : this.source;
          }
        }
        Q.RegExpSource = I;
        class M {
          constructor() {
            this._items = [], this._hasAnchors = !1, this._cached = null, this._anchorCache = { A0_G0: null, A0_G1: null, A1_G0: null, A1_G1: null };
          }
          dispose() {
            this._disposeCaches();
          }
          _disposeCaches() {
            this._cached && (this._cached.dispose(), this._cached = null), this._anchorCache.A0_G0 && (this._anchorCache.A0_G0.dispose(), this._anchorCache.A0_G0 = null), this._anchorCache.A0_G1 && (this._anchorCache.A0_G1.dispose(), this._anchorCache.A0_G1 = null), this._anchorCache.A1_G0 && (this._anchorCache.A1_G0.dispose(), this._anchorCache.A1_G0 = null), this._anchorCache.A1_G1 && (this._anchorCache.A1_G1.dispose(), this._anchorCache.A1_G1 = null);
          }
          push(A) {
            this._items.push(A), this._hasAnchors = this._hasAnchors || A.hasAnchor;
          }
          unshift(A) {
            this._items.unshift(A), this._hasAnchors = this._hasAnchors || A.hasAnchor;
          }
          length() {
            return this._items.length;
          }
          setSource(A, E) {
            this._items[A].source !== E && (this._disposeCaches(), this._items[A].setSource(E));
          }
          compile(A) {
            if (!this._cached) {
              let E = this._items.map((C) => C.source);
              this._cached = new h(A, E, this._items.map((C) => C.ruleId));
            }
            return this._cached;
          }
          compileAG(A, E, C) {
            return this._hasAnchors ? E ? C ? (this._anchorCache.A1_G1 || (this._anchorCache.A1_G1 = this._resolveAnchors(A, E, C)), this._anchorCache.A1_G1) : (this._anchorCache.A1_G0 || (this._anchorCache.A1_G0 = this._resolveAnchors(A, E, C)), this._anchorCache.A1_G0) : C ? (this._anchorCache.A0_G1 || (this._anchorCache.A0_G1 = this._resolveAnchors(A, E, C)), this._anchorCache.A0_G1) : (this._anchorCache.A0_G0 || (this._anchorCache.A0_G0 = this._resolveAnchors(A, E, C)), this._anchorCache.A0_G0) : this.compile(A);
          }
          _resolveAnchors(A, E, C) {
            let B = this._items.map((s) => s.resolveAnchors(E, C));
            return new h(A, B, this._items.map((s) => s.ruleId));
          }
        }
        Q.RegExpSourceList = M;
        class h {
          constructor(A, E, C) {
            this.regExps = E, this.rules = C, this.scanner = A.createOnigScanner(E);
          }
          dispose() {
            typeof this.scanner.dispose == "function" && this.scanner.dispose();
          }
          toString() {
            const A = [];
            for (let E = 0, C = this.rules.length; E < C; E++)
              A.push("   - " + this.rules[E] + ": " + this.regExps[E]);
            return A.join(`
`);
          }
          findNextMatchSync(A, E, C) {
            const B = this.scanner.findNextMatchSync(A, E, C);
            return B ? { ruleId: this.rules[B.index], captureIndices: B.captureIndices } : null;
          }
        }
        Q.CompiledRule = h;
      }, 583: (K, Q, N) => {
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.ThemeTrieElement = Q.ThemeTrieElementRule = Q.ColorMap = Q.fontStyleToString = Q.ParsedThemeRule = Q.parseTheme = Q.StyleAttributes = Q.ScopeStack = Q.Theme = void 0;
        const D = N(878);
        class F {
          constructor(I, M, h) {
            this._colorMap = I, this._defaults = M, this._root = h, this._cachedMatchRoot = new D.CachedFn((w) => this._root.match(w));
          }
          static createFromRawTheme(I, M) {
            return this.createFromParsedTheme(t(I), M);
          }
          static createFromParsedTheme(I, M) {
            return function(h, w) {
              h.sort((n, R) => {
                let P = D.strcmp(n.scope, R.scope);
                return P !== 0 ? P : (P = D.strArrCmp(n.parentScopes, R.parentScopes), P !== 0 ? P : n.index - R.index);
              });
              let A = 0, E = "#000000", C = "#ffffff";
              for (; h.length >= 1 && h[0].scope === ""; ) {
                let n = h.shift();
                n.fontStyle !== -1 && (A = n.fontStyle), n.foreground !== null && (E = n.foreground), n.background !== null && (C = n.background);
              }
              let B = new a(w), s = new i(A, B.getId(E), B.getId(C)), o = new H(new k(0, null, -1, 0, 0), []);
              for (let n = 0, R = h.length; n < R; n++) {
                let P = h[n];
                o.insert(0, P.scope, P.parentScopes, P.fontStyle, B.getId(P.foreground), B.getId(P.background));
              }
              return new F(B, s, o);
            }(I, M);
          }
          getColorMap() {
            return this._colorMap.getColorMap();
          }
          getDefaults() {
            return this._defaults;
          }
          match(I) {
            if (I === null)
              return this._defaults;
            const M = I.scopeName, h = this._cachedMatchRoot.get(M).find((w) => function(A, E) {
              if (E === null)
                return !0;
              let C = 0, B = E[C];
              for (; A; ) {
                if (G(A.scopeName, B)) {
                  if (C++, C === E.length)
                    return !0;
                  B = E[C];
                }
                A = A.parent;
              }
              return !1;
            }(I.parent, w.parentScopes));
            return h ? new i(h.fontStyle, h.foreground, h.background) : null;
          }
        }
        Q.Theme = F;
        class g {
          constructor(I, M) {
            this.parent = I, this.scopeName = M;
          }
          static push(I, M) {
            for (const h of M)
              I = new g(I, h);
            return I;
          }
          static from(...I) {
            let M = null;
            for (let h = 0; h < I.length; h++)
              M = new g(M, I[h]);
            return M;
          }
          push(I) {
            return new g(this, I);
          }
          getSegments() {
            let I = this;
            const M = [];
            for (; I; )
              M.push(I.scopeName), I = I.parent;
            return M.reverse(), M;
          }
          toString() {
            return this.getSegments().join(" ");
          }
          extends(I) {
            return this === I || this.parent !== null && this.parent.extends(I);
          }
          getExtensionIfDefined(I) {
            const M = [];
            let h = this;
            for (; h && h !== I; )
              M.push(h.scopeName), h = h.parent;
            return h === I ? M.reverse() : void 0;
          }
        }
        function G(e, I) {
          return I === e || e.startsWith(I) && e[I.length] === ".";
        }
        Q.ScopeStack = g;
        class i {
          constructor(I, M, h) {
            this.fontStyle = I, this.foregroundId = M, this.backgroundId = h;
          }
        }
        function t(e) {
          if (!e)
            return [];
          if (!e.settings || !Array.isArray(e.settings))
            return [];
          let I = e.settings, M = [], h = 0;
          for (let w = 0, A = I.length; w < A; w++) {
            let E, C = I[w];
            if (!C.settings)
              continue;
            if (typeof C.scope == "string") {
              let n = C.scope;
              n = n.replace(/^[,]+/, ""), n = n.replace(/[,]+$/, ""), E = n.split(",");
            } else
              E = Array.isArray(C.scope) ? C.scope : [""];
            let B = -1;
            if (typeof C.settings.fontStyle == "string") {
              B = 0;
              let n = C.settings.fontStyle.split(" ");
              for (let R = 0, P = n.length; R < P; R++)
                switch (n[R]) {
                  case "italic":
                    B |= 1;
                    break;
                  case "bold":
                    B |= 2;
                    break;
                  case "underline":
                    B |= 4;
                    break;
                  case "strikethrough":
                    B |= 8;
                }
            }
            let s = null;
            typeof C.settings.foreground == "string" && D.isValidHexColor(C.settings.foreground) && (s = C.settings.foreground);
            let o = null;
            typeof C.settings.background == "string" && D.isValidHexColor(C.settings.background) && (o = C.settings.background);
            for (let n = 0, R = E.length; n < R; n++) {
              let P = E[n].trim().split(" "), y = P[P.length - 1], S = null;
              P.length > 1 && (S = P.slice(0, P.length - 1), S.reverse()), M[h++] = new Y(y, S, w, B, s, o);
            }
          }
          return M;
        }
        Q.StyleAttributes = i, Q.parseTheme = t;
        class Y {
          constructor(I, M, h, w, A, E) {
            this.scope = I, this.parentScopes = M, this.index = h, this.fontStyle = w, this.foreground = A, this.background = E;
          }
        }
        Q.ParsedThemeRule = Y, Q.fontStyleToString = function(e) {
          if (e === -1)
            return "not set";
          let I = "";
          return 1 & e && (I += "italic "), 2 & e && (I += "bold "), 4 & e && (I += "underline "), 8 & e && (I += "strikethrough "), I === "" && (I = "none"), I.trim();
        };
        class a {
          constructor(I) {
            if (this._lastColorId = 0, this._id2color = [], this._color2id = /* @__PURE__ */ Object.create(null), Array.isArray(I)) {
              this._isFrozen = !0;
              for (let M = 0, h = I.length; M < h; M++)
                this._color2id[I[M]] = M, this._id2color[M] = I[M];
            } else
              this._isFrozen = !1;
          }
          getId(I) {
            if (I === null)
              return 0;
            I = I.toUpperCase();
            let M = this._color2id[I];
            if (M)
              return M;
            if (this._isFrozen)
              throw new Error(`Missing color in color map - ${I}`);
            return M = ++this._lastColorId, this._color2id[I] = M, this._id2color[M] = I, M;
          }
          getColorMap() {
            return this._id2color.slice(0);
          }
        }
        Q.ColorMap = a;
        class k {
          constructor(I, M, h, w, A) {
            this.scopeDepth = I, this.parentScopes = M, this.fontStyle = h, this.foreground = w, this.background = A;
          }
          clone() {
            return new k(this.scopeDepth, this.parentScopes, this.fontStyle, this.foreground, this.background);
          }
          static cloneArr(I) {
            let M = [];
            for (let h = 0, w = I.length; h < w; h++)
              M[h] = I[h].clone();
            return M;
          }
          acceptOverwrite(I, M, h, w) {
            this.scopeDepth > I ? console.log("how did this happen?") : this.scopeDepth = I, M !== -1 && (this.fontStyle = M), h !== 0 && (this.foreground = h), w !== 0 && (this.background = w);
          }
        }
        Q.ThemeTrieElementRule = k;
        class H {
          constructor(I, M = [], h = {}) {
            this._mainRule = I, this._children = h, this._rulesWithParentScopes = M;
          }
          static _sortBySpecificity(I) {
            return I.length === 1 || I.sort(this._cmpBySpecificity), I;
          }
          static _cmpBySpecificity(I, M) {
            if (I.scopeDepth === M.scopeDepth) {
              const h = I.parentScopes, w = M.parentScopes;
              let A = h === null ? 0 : h.length, E = w === null ? 0 : w.length;
              if (A === E)
                for (let C = 0; C < A; C++) {
                  const B = h[C].length, s = w[C].length;
                  if (B !== s)
                    return s - B;
                }
              return E - A;
            }
            return M.scopeDepth - I.scopeDepth;
          }
          match(I) {
            if (I === "")
              return H._sortBySpecificity([].concat(this._mainRule).concat(this._rulesWithParentScopes));
            let M, h, w = I.indexOf(".");
            return w === -1 ? (M = I, h = "") : (M = I.substring(0, w), h = I.substring(w + 1)), this._children.hasOwnProperty(M) ? this._children[M].match(h) : H._sortBySpecificity([].concat(this._mainRule).concat(this._rulesWithParentScopes));
          }
          insert(I, M, h, w, A, E) {
            if (M === "")
              return void this._doInsertHere(I, h, w, A, E);
            let C, B, s, o = M.indexOf(".");
            o === -1 ? (C = M, B = "") : (C = M.substring(0, o), B = M.substring(o + 1)), this._children.hasOwnProperty(C) ? s = this._children[C] : (s = new H(this._mainRule.clone(), k.cloneArr(this._rulesWithParentScopes)), this._children[C] = s), s.insert(I + 1, B, h, w, A, E);
          }
          _doInsertHere(I, M, h, w, A) {
            if (M !== null) {
              for (let E = 0, C = this._rulesWithParentScopes.length; E < C; E++) {
                let B = this._rulesWithParentScopes[E];
                if (D.strArrCmp(B.parentScopes, M) === 0)
                  return void B.acceptOverwrite(I, h, w, A);
              }
              h === -1 && (h = this._mainRule.fontStyle), w === 0 && (w = this._mainRule.foreground), A === 0 && (A = this._mainRule.background), this._rulesWithParentScopes.push(new k(I, M, h, w, A));
            } else
              this._mainRule.acceptOverwrite(I, h, w, A);
          }
        }
        Q.ThemeTrieElement = H;
      }, 878: (K, Q) => {
        function N(g) {
          return Array.isArray(g) ? function(G) {
            let i = [];
            for (let t = 0, Y = G.length; t < Y; t++)
              i[t] = N(G[t]);
            return i;
          }(g) : typeof g == "object" ? function(G) {
            let i = {};
            for (let t in G)
              i[t] = N(G[t]);
            return i;
          }(g) : g;
        }
        Object.defineProperty(Q, "__esModule", { value: !0 }), Q.performanceNow = Q.CachedFn = Q.escapeRegExpCharacters = Q.isValidHexColor = Q.strArrCmp = Q.strcmp = Q.RegexSource = Q.basename = Q.mergeObjects = Q.clone = void 0, Q.clone = function(g) {
          return N(g);
        }, Q.mergeObjects = function(g, ...G) {
          return G.forEach((i) => {
            for (let t in i)
              g[t] = i[t];
          }), g;
        }, Q.basename = function g(G) {
          const i = ~G.lastIndexOf("/") || ~G.lastIndexOf("\\");
          return i === 0 ? G : ~i == G.length - 1 ? g(G.substring(0, G.length - 1)) : G.substr(1 + ~i);
        };
        let D = /\$(\d+)|\${(\d+):\/(downcase|upcase)}/g;
        function F(g, G) {
          return g < G ? -1 : g > G ? 1 : 0;
        }
        Q.RegexSource = class {
          static hasCaptures(g) {
            return g !== null && (D.lastIndex = 0, D.test(g));
          }
          static replaceCaptures(g, G, i) {
            return g.replace(D, (t, Y, a, k) => {
              let H = i[parseInt(Y || a, 10)];
              if (!H)
                return t;
              {
                let e = G.substring(H.start, H.end);
                for (; e[0] === "."; )
                  e = e.substring(1);
                switch (k) {
                  case "downcase":
                    return e.toLowerCase();
                  case "upcase":
                    return e.toUpperCase();
                  default:
                    return e;
                }
              }
            });
          }
        }, Q.strcmp = F, Q.strArrCmp = function(g, G) {
          if (g === null && G === null)
            return 0;
          if (!g)
            return -1;
          if (!G)
            return 1;
          let i = g.length, t = G.length;
          if (i === t) {
            for (let Y = 0; Y < i; Y++) {
              let a = F(g[Y], G[Y]);
              if (a !== 0)
                return a;
            }
            return 0;
          }
          return i - t;
        }, Q.isValidHexColor = function(g) {
          return !!(/^#[0-9a-f]{6}$/i.test(g) || /^#[0-9a-f]{8}$/i.test(g) || /^#[0-9a-f]{3}$/i.test(g) || /^#[0-9a-f]{4}$/i.test(g));
        }, Q.escapeRegExpCharacters = function(g) {
          return g.replace(/[\-\\\{\}\*\+\?\|\^\$\.\,\[\]\(\)\#\s]/g, "\\$&");
        }, Q.CachedFn = class {
          constructor(g) {
            this.fn = g, this.cache = /* @__PURE__ */ new Map();
          }
          get(g) {
            if (this.cache.has(g))
              return this.cache.get(g);
            const G = this.fn(g);
            return this.cache.set(g, G), G;
          }
        }, Q.performanceNow = typeof performance > "u" ? function() {
          return Date.now();
        } : function() {
          return performance.now();
        };
      } }, r = {};
      return function K(Q) {
        var N = r[Q];
        if (N !== void 0)
          return N.exports;
        var D = r[Q] = { exports: {} };
        return L[Q].call(D.exports, D, D.exports, K), D.exports;
      }(787);
    })();
  });
})(EB);
var KA = EB.exports;
const VB = /* @__PURE__ */ XA(KA), jB = /* @__PURE__ */ gB({
  __proto__: null,
  default: VB
}, [KA]), YA = class YA extends LA {
  constructor(c, L, r, K, Q, N) {
    super(), this._model = c, this._worker = L, this._languageIdCodec = r, this._backgroundTokenizationStore = K, this._configurationService = Q, this._maxTokenizationLineLength = N, this.controllerId = YA._id++, this._pendingChanges = [], this._states = new eB(), this._loggingEnabled = _B("editor.experimental.asyncTokenizationLogging", !1, this._configurationService), this._register(TA(this._loggingEnabled)), this._register(this._model.onDidChangeContent((g) => {
      this._shouldLog && console.log("model change", {
        fileName: this._model.uri.fsPath.split("\\").pop(),
        changes: PA(g.changes)
      }), this._worker.acceptModelChanged(this.controllerId, g), this._pendingChanges.push(g);
    })), this._register(this._model.onDidChangeLanguage((g) => {
      const G = this._model.getLanguageId(), i = this._languageIdCodec.encodeLanguageId(G);
      this._worker.acceptModelLanguageChanged(this.controllerId, G, i);
    }));
    const D = this._model.getLanguageId(), F = this._languageIdCodec.encodeLanguageId(D);
    this._worker.acceptNewModel({
      uri: this._model.uri,
      versionId: this._model.getVersionId(),
      lines: this._model.getLinesContent(),
      EOL: this._model.getEOL(),
      languageId: D,
      encodedLanguageId: F,
      maxTokenizationLineLength: this._maxTokenizationLineLength.get(),
      controllerId: this.controllerId
    }), this._register(cB((g) => {
      const G = this._maxTokenizationLineLength.read(g);
      this._worker.acceptMaxTokenizationLineLength(this.controllerId, G);
    }));
  }
  dispose() {
    super.dispose(), this._worker.acceptRemovedModel(this.controllerId);
  }
  requestTokens(c, L) {
    this._worker.retokenize(this.controllerId, c, L);
  }
  async setTokensAndStates(c, L, r, K) {
    if (this.controllerId !== c)
      return;
    let Q = OA.deserialize(new Uint8Array(r));
    if (this._shouldLog && console.log("received background tokenization result", {
      fileName: this._model.uri.fsPath.split("\\").pop(),
      updatedTokenLines: Q.map((D) => D.getLineRange()).join(" & "),
      updatedStateLines: K.map((D) => new tB(D.startLineNumber, D.startLineNumber + D.stateDeltas.length).toString()).join(" & ")
    }), this._shouldLog) {
      const D = this._pendingChanges.filter((F) => F.versionId <= L).map((F) => F.changes).map((F) => PA(F)).join(" then ");
      console.log("Applying changes to local states", D);
    }
    for (; this._pendingChanges.length > 0 && this._pendingChanges[0].versionId <= L; ) {
      const D = this._pendingChanges.shift();
      this._states.acceptChanges(D.changes);
    }
    if (this._pendingChanges.length > 0) {
      if (this._shouldLog) {
        const g = this._pendingChanges.map((G) => G.changes).map((G) => PA(G)).join(" then ");
        console.log("Considering non-processed changes", g);
      }
      const D = RA.fromMany(this._pendingChanges.map((g) => mA(g.changes))), F = new OA();
      for (const g of Q)
        for (let G = g.startLineNumber; G <= g.endLineNumber; G++)
          D.transform(G - 1) !== void 0 && F.add(G, g.getLineTokens(G));
      Q = F.finalize();
      for (const g of this._pendingChanges)
        for (const G of g.changes)
          for (let i = 0; i < Q.length; i++)
            Q[i].applyEdit(G.range, G.text);
    }
    const N = RA.fromMany(this._pendingChanges.map((D) => mA(D.changes)));
    (!this._applyStateStackDiffFn || !this._initialState) && (this._applyStateStackDiffFn = KA.applyStateStackDiff, this._initialState = KA.INITIAL);
    for (const D of K) {
      let F = D.startLineNumber <= 1 ? this._initialState : this._states.getEndState(D.startLineNumber - 1);
      for (let g = 0; g < D.stateDeltas.length; g++) {
        const G = D.stateDeltas[g];
        let i;
        G ? (i = this._applyStateStackDiffFn(F, G), this._states.setEndState(D.startLineNumber + g, i)) : i = this._states.getEndState(D.startLineNumber + g);
        const t = N.transform(D.startLineNumber + g - 1);
        t !== void 0 && this._backgroundTokenizationStore.setEndState(t + 1, i), D.startLineNumber + g >= this._model.getLineCount() - 1 && this._backgroundTokenizationStore.backgroundTokenizationFinished(), F = i;
      }
    }
    this._backgroundTokenizationStore.setTokens(Q);
  }
  get _shouldLog() {
    return this._loggingEnabled.get();
  }
};
YA._id = 0;
let yA = YA;
function mA(p) {
  return new vB(p.map((c) => new ZB(
    c.range.startLineNumber - 1,
    c.range.endLineNumber - c.range.startLineNumber + 1,
    MB(c.text)[0] + 1
  )));
}
function PA(p) {
  return p.map((c) => hB.lift(c.range).toString() + " => " + c.text).join(" & ");
}
function _B(p, c, L) {
  return vA((r) => L.onDidChangeConfiguration((K) => {
    K.affectsConfiguration(p) && r(K);
  }), () => L.getValue(p) ?? c);
}
var HA, FA;
let SA = (FA = class {
  constructor(c, L, r, K, Q, N, D, F, g, G) {
    this._reportTokenizationTime = c, this._shouldTokenizeAsync = L, this._extensionResourceLoaderService = r, this._modelService = K, this._languageConfigurationService = Q, this._configurationService = N, this._languageService = D, this._environmentService = F, this._notificationService = g, this._telemetryService = G, this._workerProxyPromise = null, this._worker = null, this._workerProxy = null, this._workerTokenizerControllers = /* @__PURE__ */ new Map(), this._currentTheme = null, this._currentTokenColorMap = null, this._grammarDefinitions = [];
  }
  dispose() {
    this._disposeWorker();
  }
  createBackgroundTokenizer(c, L, r) {
    if (!this._shouldTokenizeAsync() || c.isTooLargeForSyncing())
      return;
    const K = new rA(), Q = this._getWorkerProxy().then((N) => {
      if (K.isDisposed || !N)
        return;
      const D = { controller: void 0, worker: this._worker };
      return K.add($B(c, () => {
        const F = new yA(
          c,
          N,
          this._languageService.languageIdCodec,
          L,
          this._configurationService,
          r
        );
        return D.controller = F, this._workerTokenizerControllers.set(F.controllerId, F), FB(() => {
          D.controller = void 0, this._workerTokenizerControllers.delete(F.controllerId), F.dispose();
        });
      })), D;
    });
    return {
      dispose() {
        K.dispose();
      },
      requestTokens: async (N, D) => {
        const F = await Q;
        F != null && F.controller && F.worker === this._worker && F.controller.requestTokens(N, D);
      },
      reportMismatchingTokens: (N) => {
        HA._reportedMismatchingTokens || (HA._reportedMismatchingTokens = !0, this._notificationService.error({
          message: "Async Tokenization Token Mismatch in line " + N,
          name: "Async Tokenization Token Mismatch"
        }), this._telemetryService.publicLog2("asyncTokenizationMismatchingTokens", {}));
      }
    };
  }
  setGrammarDefinitions(c) {
    this._grammarDefinitions = c, this._disposeWorker();
  }
  acceptTheme(c, L) {
    this._currentTheme = c, this._currentTokenColorMap = L, this._currentTheme && this._currentTokenColorMap && this._workerProxy && this._workerProxy.acceptTheme(this._currentTheme, this._currentTokenColorMap);
  }
  _getWorkerProxy() {
    return this._workerProxyPromise || (this._workerProxyPromise = this._createWorkerProxy()), this._workerProxyPromise;
  }
  async _createWorkerProxy() {
    const c = `${JA}/vscode-textmate`, L = `${xA}/vscode-textmate`, r = `${JA}/vscode-oniguruma`, K = `${xA}/vscode-oniguruma`, Q = this._environmentService.isBuilt && !ZA, N = Q ? L : c, D = Q ? K : r, F = `${N}/release/main.js`, g = `${D}/release/main.js`, G = `${D}/release/onig.wasm`, i = aA.asBrowserUri(F).toString(!0), t = {
      grammarDefinitions: this._grammarDefinitions,
      textmateMainUri: i,
      onigurumaMainUri: aA.asBrowserUri(g).toString(!0),
      onigurumaWASMUri: aA.asBrowserUri(G).toString(!0)
    }, Y = {
      readFile: async (H) => {
        const e = nB.revive(H);
        return this._extensionResourceLoaderService.readExtensionResource(e);
      },
      setTokensAndStates: async (H, e, I, M) => {
        const h = this._workerTokenizerControllers.get(H);
        h && h.setTokensAndStates(H, e, I, M);
      },
      reportTokenizationTime: (H, e, I, M, h) => {
        this._reportTokenizationTime(H, e, I, M, h);
      }
    }, a = this._worker = aB(this._modelService, this._languageConfigurationService, {
      createData: t,
      label: "textMateWorker",
      moduleId: "vs/workbench/services/textMate/browser/backgroundTokenization/worker/textMateTokenizationWorker.worker",
      host: Y
    }), k = await a.getProxy();
    return this._worker !== a ? null : (this._workerProxy = k, this._currentTheme && this._currentTokenColorMap && this._workerProxy.acceptTheme(this._currentTheme, this._currentTokenColorMap), k);
  }
  _disposeWorker() {
    for (const c of this._workerTokenizerControllers.values())
      c.dispose();
    this._workerTokenizerControllers.clear(), this._worker && (this._worker.dispose(), this._worker = null), this._workerProxy = null, this._workerProxyPromise = null;
  }
}, HA = FA, FA._reportedMismatchingTokens = !1, FA);
SA = HA = pA([
  V(2, zA),
  V(3, GB),
  V(4, NB),
  V(5, VA),
  V(6, jA),
  V(7, LB),
  V(8, _A),
  V(9, $A)
], SA);
function $B(p, c) {
  const L = new rA(), r = L.add(new rA());
  function K() {
    p.isAttachedToEditor() ? r.add(c()) : r.clear();
  }
  return K(), L.add(p.onDidChangeAttached(() => {
    K();
  })), L;
}
class AQ {
  constructor() {
    this._scopeNameToLanguageRegistration = /* @__PURE__ */ Object.create(null);
  }
  reset() {
    this._scopeNameToLanguageRegistration = /* @__PURE__ */ Object.create(null);
  }
  register(c) {
    this._scopeNameToLanguageRegistration[c.scopeName] = c;
  }
  getGrammarDefinition(c) {
    return this._scopeNameToLanguageRegistration[c] || null;
  }
}
const kA = "No TM Grammar registered for this language.";
class BQ extends LA {
  constructor(c, L, r, K) {
    super(), this._host = c, this._initialState = r.INITIAL, this._scopeRegistry = new AQ(), this._injections = {}, this._injectedEmbeddedLanguages = {}, this._languageToScope = /* @__PURE__ */ new Map(), this._grammarRegistry = this._register(new r.Registry({
      onigLib: K,
      loadGrammar: async (Q) => {
        const N = this._scopeRegistry.getGrammarDefinition(Q);
        if (!N)
          return this._host.logTrace(`No grammar found for scope ${Q}`), null;
        const D = N.location;
        try {
          const F = await this._host.readFile(D);
          return r.parseRawGrammar(F, D.path);
        } catch (F) {
          return this._host.logError(`Unable to load and parse grammar for scope ${Q} from ${D}`, F), null;
        }
      },
      getInjections: (Q) => {
        const N = Q.split(".");
        let D = [];
        for (let F = 1; F <= N.length; F++) {
          const g = N.slice(0, F).join(".");
          D = [...D, ...this._injections[g] || []];
        }
        return D;
      }
    }));
    for (const Q of L) {
      if (this._scopeRegistry.register(Q), Q.injectTo) {
        for (const N of Q.injectTo) {
          let D = this._injections[N];
          D || (this._injections[N] = D = []), D.push(Q.scopeName);
        }
        if (Q.embeddedLanguages)
          for (const N of Q.injectTo) {
            let D = this._injectedEmbeddedLanguages[N];
            D || (this._injectedEmbeddedLanguages[N] = D = []), D.push(Q.embeddedLanguages);
          }
      }
      Q.language && this._languageToScope.set(Q.language, Q.scopeName);
    }
  }
  has(c) {
    return this._languageToScope.has(c);
  }
  setTheme(c, L) {
    this._grammarRegistry.setTheme(c, L);
  }
  getColorMap() {
    return this._grammarRegistry.getColorMap();
  }
  async createGrammar(c, L) {
    const r = this._languageToScope.get(c);
    if (typeof r != "string")
      throw new Error(kA);
    const K = this._scopeRegistry.getGrammarDefinition(r);
    if (!K)
      throw new Error(kA);
    const Q = K.embeddedLanguages;
    if (this._injectedEmbeddedLanguages[r]) {
      const F = this._injectedEmbeddedLanguages[r];
      for (const g of F)
        for (const G of Object.keys(g))
          Q[G] = g[G];
    }
    const N = Object.keys(Q).length > 0;
    let D;
    try {
      D = await this._grammarRegistry.loadGrammarWithConfiguration(r, L, {
        embeddedLanguages: Q,
        tokenTypes: K.tokenTypes,
        balancedBracketSelectors: K.balancedBracketSelectors,
        unbalancedBracketSelectors: K.unbalancedBracketSelectors
      });
    } catch (F) {
      throw F.message && F.message.startsWith("No grammar provided for") ? new Error(kA) : F;
    }
    return {
      languageId: c,
      grammar: D,
      initialState: this._initialState,
      containsEmbeddedLanguages: N,
      sourceExtensionId: K.sourceExtensionId
    };
  }
}
const IA = HB.registerExtensionPoint({
  extensionPoint: "grammars",
  deps: [kB],
  jsonSchema: {
    description: z(
      "vs/workbench/services/textMate/common/TMGrammars",
      "vscode.extension.contributes.grammars",
      "Contributes textmate tokenizers."
    ),
    type: "array",
    defaultSnippets: [{ body: [{ language: "${1:id}", scopeName: "source.${2:id}", path: "./syntaxes/${3:id}.tmLanguage." }] }],
    items: {
      type: "object",
      defaultSnippets: [{ body: { language: "${1:id}", scopeName: "source.${2:id}", path: "./syntaxes/${3:id}.tmLanguage." } }],
      properties: {
        language: {
          description: z(
            "vs/workbench/services/textMate/common/TMGrammars",
            "vscode.extension.contributes.grammars.language",
            "Language identifier for which this syntax is contributed to."
          ),
          type: "string"
        },
        scopeName: {
          description: z(
            "vs/workbench/services/textMate/common/TMGrammars",
            "vscode.extension.contributes.grammars.scopeName",
            "Textmate scope name used by the tmLanguage file."
          ),
          type: "string"
        },
        path: {
          description: z(
            "vs/workbench/services/textMate/common/TMGrammars",
            "vscode.extension.contributes.grammars.path",
            "Path of the tmLanguage file. The path is relative to the extension folder and typically starts with './syntaxes/'."
          ),
          type: "string"
        },
        embeddedLanguages: {
          description: z(
            "vs/workbench/services/textMate/common/TMGrammars",
            "vscode.extension.contributes.grammars.embeddedLanguages",
            "A map of scope name to language id if this grammar contains embedded languages."
          ),
          type: "object"
        },
        tokenTypes: {
          description: z(
            "vs/workbench/services/textMate/common/TMGrammars",
            "vscode.extension.contributes.grammars.tokenTypes",
            "A map of scope name to token types."
          ),
          type: "object",
          additionalProperties: {
            enum: ["string", "comment", "other"]
          }
        },
        injectTo: {
          description: z(
            "vs/workbench/services/textMate/common/TMGrammars",
            "vscode.extension.contributes.grammars.injectTo",
            "List of language scope names to which this grammar is injected to."
          ),
          type: "array",
          items: {
            type: "string"
          }
        },
        balancedBracketScopes: {
          description: z(
            "vs/workbench/services/textMate/common/TMGrammars",
            "vscode.extension.contributes.grammars.balancedBracketScopes",
            "Defines which scope names contain balanced brackets."
          ),
          type: "array",
          items: {
            type: "string"
          },
          default: ["*"]
        },
        unbalancedBracketScopes: {
          description: z(
            "vs/workbench/services/textMate/common/TMGrammars",
            "vscode.extension.contributes.grammars.unbalancedBracketScopes",
            "Defines which scope names do not contain balanced brackets."
          ),
          type: "array",
          items: {
            type: "string"
          },
          default: []
        }
      },
      required: ["scopeName", "path"]
    }
  }
});
var CB = { exports: {} };
(function(p, c) {
  (function(L, r) {
    p.exports = r();
  })(WA, () => {
    return L = { 770: function(K, Q, N) {
      var D = this && this.__importDefault || function(H) {
        return H && H.__esModule ? H : { default: H };
      };
      Object.defineProperty(Q, "__esModule", { value: !0 }), Q.setDefaultDebugCall = Q.createOnigScanner = Q.createOnigString = Q.loadWASM = Q.OnigScanner = Q.OnigString = void 0;
      const F = D(N(418));
      let g = null, G = !1;
      class i {
        static _utf8ByteLength(e) {
          let I = 0;
          for (let M = 0, h = e.length; M < h; M++) {
            const w = e.charCodeAt(M);
            let A = w, E = !1;
            if (w >= 55296 && w <= 56319 && M + 1 < h) {
              const C = e.charCodeAt(M + 1);
              C >= 56320 && C <= 57343 && (A = 65536 + (w - 55296 << 10) | C - 56320, E = !0);
            }
            I += A <= 127 ? 1 : A <= 2047 ? 2 : A <= 65535 ? 3 : 4, E && M++;
          }
          return I;
        }
        constructor(e) {
          const I = e.length, M = i._utf8ByteLength(e), h = M !== I, w = h ? new Uint32Array(I + 1) : null;
          h && (w[I] = M);
          const A = h ? new Uint32Array(M + 1) : null;
          h && (A[M] = I);
          const E = new Uint8Array(M);
          let C = 0;
          for (let B = 0; B < I; B++) {
            const s = e.charCodeAt(B);
            let o = s, n = !1;
            if (s >= 55296 && s <= 56319 && B + 1 < I) {
              const R = e.charCodeAt(B + 1);
              R >= 56320 && R <= 57343 && (o = 65536 + (s - 55296 << 10) | R - 56320, n = !0);
            }
            h && (w[B] = C, n && (w[B + 1] = C), o <= 127 ? A[C + 0] = B : o <= 2047 ? (A[C + 0] = B, A[C + 1] = B) : o <= 65535 ? (A[C + 0] = B, A[C + 1] = B, A[C + 2] = B) : (A[C + 0] = B, A[C + 1] = B, A[C + 2] = B, A[C + 3] = B)), o <= 127 ? E[C++] = o : o <= 2047 ? (E[C++] = 192 | (1984 & o) >>> 6, E[C++] = 128 | (63 & o) >>> 0) : o <= 65535 ? (E[C++] = 224 | (61440 & o) >>> 12, E[C++] = 128 | (4032 & o) >>> 6, E[C++] = 128 | (63 & o) >>> 0) : (E[C++] = 240 | (1835008 & o) >>> 18, E[C++] = 128 | (258048 & o) >>> 12, E[C++] = 128 | (4032 & o) >>> 6, E[C++] = 128 | (63 & o) >>> 0), n && B++;
          }
          this.utf16Length = I, this.utf8Length = M, this.utf16Value = e, this.utf8Value = E, this.utf16OffsetToUtf8 = w, this.utf8OffsetToUtf16 = A;
        }
        createString(e) {
          const I = e._omalloc(this.utf8Length);
          return e.HEAPU8.set(this.utf8Value, I), I;
        }
      }
      class t {
        constructor(e) {
          if (this.id = ++t.LAST_ID, !g)
            throw new Error("Must invoke loadWASM first.");
          this._onigBinding = g, this.content = e;
          const I = new i(e);
          this.utf16Length = I.utf16Length, this.utf8Length = I.utf8Length, this.utf16OffsetToUtf8 = I.utf16OffsetToUtf8, this.utf8OffsetToUtf16 = I.utf8OffsetToUtf16, this.utf8Length < 1e4 && !t._sharedPtrInUse ? (t._sharedPtr || (t._sharedPtr = g._omalloc(1e4)), t._sharedPtrInUse = !0, g.HEAPU8.set(I.utf8Value, t._sharedPtr), this.ptr = t._sharedPtr) : this.ptr = I.createString(g);
        }
        convertUtf8OffsetToUtf16(e) {
          return this.utf8OffsetToUtf16 ? e < 0 ? 0 : e > this.utf8Length ? this.utf16Length : this.utf8OffsetToUtf16[e] : e;
        }
        convertUtf16OffsetToUtf8(e) {
          return this.utf16OffsetToUtf8 ? e < 0 ? 0 : e > this.utf16Length ? this.utf8Length : this.utf16OffsetToUtf8[e] : e;
        }
        dispose() {
          this.ptr === t._sharedPtr ? t._sharedPtrInUse = !1 : this._onigBinding._ofree(this.ptr);
        }
      }
      Q.OnigString = t, t.LAST_ID = 0, t._sharedPtr = 0, t._sharedPtrInUse = !1;
      class Y {
        constructor(e) {
          if (!g)
            throw new Error("Must invoke loadWASM first.");
          const I = [], M = [];
          for (let E = 0, C = e.length; E < C; E++) {
            const B = new i(e[E]);
            I[E] = B.createString(g), M[E] = B.utf8Length;
          }
          const h = g._omalloc(4 * e.length);
          g.HEAPU32.set(I, h / 4);
          const w = g._omalloc(4 * e.length);
          g.HEAPU32.set(M, w / 4);
          const A = g._createOnigScanner(h, w, e.length);
          for (let E = 0, C = e.length; E < C; E++)
            g._ofree(I[E]);
          g._ofree(w), g._ofree(h), A === 0 && function(E) {
            throw new Error(E.UTF8ToString(E._getLastOnigError()));
          }(g), this._onigBinding = g, this._ptr = A;
        }
        dispose() {
          this._onigBinding._freeOnigScanner(this._ptr);
        }
        findNextMatchSync(e, I, M) {
          let h = G, w = 0;
          if (typeof M == "number" ? (8 & M && (h = !0), w = M) : typeof M == "boolean" && (h = M), typeof e == "string") {
            e = new t(e);
            const A = this._findNextMatchSync(e, I, h, w);
            return e.dispose(), A;
          }
          return this._findNextMatchSync(e, I, h, w);
        }
        _findNextMatchSync(e, I, M, h) {
          const w = this._onigBinding;
          let A;
          if (A = M ? w._findNextOnigScannerMatchDbg(this._ptr, e.id, e.ptr, e.utf8Length, e.convertUtf16OffsetToUtf8(I), h) : w._findNextOnigScannerMatch(this._ptr, e.id, e.ptr, e.utf8Length, e.convertUtf16OffsetToUtf8(I), h), A === 0)
            return null;
          const E = w.HEAPU32;
          let C = A / 4;
          const B = E[C++], s = E[C++];
          let o = [];
          for (let n = 0; n < s; n++) {
            const R = e.convertUtf8OffsetToUtf16(E[C++]), P = e.convertUtf8OffsetToUtf16(E[C++]);
            o[n] = { start: R, end: P, length: P - R };
          }
          return { index: B, captureIndices: o };
        }
      }
      Q.OnigScanner = Y;
      let a = !1, k = null;
      Q.loadWASM = function(H) {
        if (a)
          return k;
        let e, I, M, h;
        if (a = !0, function(w) {
          return typeof w.instantiator == "function";
        }(H))
          e = H.instantiator, I = H.print;
        else {
          let w;
          (function(A) {
            return A.data !== void 0;
          })(H) ? (w = H.data, I = H.print) : w = H, e = function(A) {
            return typeof Response < "u" && A instanceof Response;
          }(w) ? typeof WebAssembly.instantiateStreaming == "function" ? /* @__PURE__ */ function(A) {
            return (E) => WebAssembly.instantiateStreaming(A, E);
          }(w) : /* @__PURE__ */ function(A) {
            return async (E) => {
              const C = await A.arrayBuffer();
              return WebAssembly.instantiate(C, E);
            };
          }(w) : /* @__PURE__ */ function(A) {
            return (E) => WebAssembly.instantiate(A, E);
          }(w);
        }
        return k = new Promise((w, A) => {
          M = w, h = A;
        }), function(w, A, E, C) {
          (0, F.default)({ print: A, instantiateWasm: (B, s) => {
            if (typeof performance > "u") {
              const o = () => Date.now();
              B.env.emscripten_get_now = o, B.wasi_snapshot_preview1.emscripten_get_now = o;
            }
            return w(B).then((o) => s(o.instance), C), {};
          } }).then((B) => {
            g = B, E();
          });
        }(e, I, M, h), k;
      }, Q.createOnigString = function(H) {
        return new t(H);
      }, Q.createOnigScanner = function(H) {
        return new Y(H);
      }, Q.setDefaultDebugCall = function(H) {
        G = H;
      };
    }, 418: (K) => {
      var Q = (typeof document < "u" && document.currentScript && document.currentScript.src, function(N) {
        var D, F, g = (N = N || {}) !== void 0 ? N : {};
        g.ready = new Promise(function(J, u) {
          D = J, F = u;
        });
        var G, i = Object.assign({}, g), t = !1, Y = "";
        function a(J) {
          return g.locateFile ? g.locateFile(J, Y) : Y + J;
        }
        G = function(J) {
          let u;
          return typeof readbuffer == "function" ? new Uint8Array(readbuffer(J)) : (u = read(J, "binary"), h(typeof u == "object"), u);
        }, typeof scriptArgs < "u" && scriptArgs, typeof onig_print < "u" && (typeof console > "u" && (console = {}), console.log = onig_print, console.warn = console.error = typeof printErr < "u" ? printErr : onig_print);
        var k, H, e = g.print || console.log.bind(console), I = g.printErr || console.warn.bind(console);
        Object.assign(g, i), i = null, g.arguments && g.arguments, g.thisProgram && g.thisProgram, g.quit && g.quit, g.wasmBinary && (k = g.wasmBinary), g.noExitRuntime, typeof WebAssembly != "object" && f("no native wasm support detected");
        var M = !1;
        function h(J, u) {
          J || f(u);
        }
        var w, A, E, C = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0;
        function B(J, u, _) {
          for (var QA = u + _, T = u; J[T] && !(T >= QA); )
            ++T;
          if (T - u > 16 && J.buffer && C)
            return C.decode(J.subarray(u, T));
          for (var m = ""; u < T; ) {
            var $ = J[u++];
            if (128 & $) {
              var tA = 63 & J[u++];
              if ((224 & $) != 192) {
                var MA = 63 & J[u++];
                if (($ = (240 & $) == 224 ? (15 & $) << 12 | tA << 6 | MA : (7 & $) << 18 | tA << 12 | MA << 6 | 63 & J[u++]) < 65536)
                  m += String.fromCharCode($);
                else {
                  var lA = $ - 65536;
                  m += String.fromCharCode(55296 | lA >> 10, 56320 | 1023 & lA);
                }
              } else
                m += String.fromCharCode((31 & $) << 6 | tA);
            } else
              m += String.fromCharCode($);
          }
          return m;
        }
        function s(J, u) {
          return J ? B(A, J, u) : "";
        }
        function o(J) {
          w = J, g.HEAP8 = new Int8Array(J), g.HEAP16 = new Int16Array(J), g.HEAP32 = new Int32Array(J), g.HEAPU8 = A = new Uint8Array(J), g.HEAPU16 = new Uint16Array(J), g.HEAPU32 = E = new Uint32Array(J), g.HEAPF32 = new Float32Array(J), g.HEAPF64 = new Float64Array(J);
        }
        g.INITIAL_MEMORY;
        var n = [], R = [], P = [];
        function y() {
          if (g.preRun)
            for (typeof g.preRun == "function" && (g.preRun = [g.preRun]); g.preRun.length; )
              q(g.preRun.shift());
          gA(n);
        }
        function S() {
          gA(R);
        }
        function W() {
          if (g.postRun)
            for (typeof g.postRun == "function" && (g.postRun = [g.postRun]); g.postRun.length; )
              X(g.postRun.shift());
          gA(P);
        }
        function q(J) {
          n.unshift(J);
        }
        function d(J) {
          R.unshift(J);
        }
        function X(J) {
          P.unshift(J);
        }
        var v = 0, b = null;
        function O(J) {
          v++, g.monitorRunDependencies && g.monitorRunDependencies(v);
        }
        function l(J) {
          if (v--, g.monitorRunDependencies && g.monitorRunDependencies(v), v == 0 && b) {
            var u = b;
            b = null, u();
          }
        }
        function f(J) {
          g.onAbort && g.onAbort(J), I(J = "Aborted(" + J + ")"), M = !0, J += ". Build with -sASSERTIONS for more info.";
          var u = new WebAssembly.RuntimeError(J);
          throw F(u), u;
        }
        var j, U, Z = "data:application/octet-stream;base64,";
        function x(J) {
          return J.startsWith(Z);
        }
        function BA(J) {
          try {
            if (J == j && k)
              return new Uint8Array(k);
            if (G)
              return G(J);
            throw "both async and sync fetching of the wasm failed";
          } catch (u) {
            f(u);
          }
        }
        function DA() {
          return k || !t || typeof fetch != "function" ? Promise.resolve().then(function() {
            return BA(j);
          }) : fetch(j, { credentials: "same-origin" }).then(function(J) {
            if (!J.ok)
              throw "failed to load wasm binary file at '" + j + "'";
            return J.arrayBuffer();
          }).catch(function() {
            return BA(j);
          });
        }
        function GA() {
          var J = { env: iA, wasi_snapshot_preview1: iA };
          function u(T, m) {
            var $ = T.exports;
            g.asm = $, o((H = g.asm.memory).buffer), g.asm.__indirect_function_table, d(g.asm.__wasm_call_ctors), l();
          }
          function _(T) {
            u(T.instance);
          }
          function QA(T) {
            return DA().then(function(m) {
              return WebAssembly.instantiate(m, J);
            }).then(function(m) {
              return m;
            }).then(T, function(m) {
              I("failed to asynchronously prepare wasm: " + m), f(m);
            });
          }
          if (O(), g.instantiateWasm)
            try {
              return g.instantiateWasm(J, u);
            } catch (T) {
              I("Module.instantiateWasm callback failed with error: " + T), F(T);
            }
          return (k || typeof WebAssembly.instantiateStreaming != "function" || x(j) || typeof fetch != "function" ? QA(_) : fetch(j, { credentials: "same-origin" }).then(function(T) {
            return WebAssembly.instantiateStreaming(T, J).then(_, function(m) {
              return I("wasm streaming compile failed: " + m), I("falling back to ArrayBuffer instantiation"), QA(_);
            });
          })).catch(F), {};
        }
        function gA(J) {
          for (; J.length > 0; )
            J.shift()(g);
        }
        function EA(J, u, _) {
          A.copyWithin(J, u, u + _);
        }
        function eA(J) {
          try {
            return H.grow(J - w.byteLength + 65535 >>> 16), o(H.buffer), 1;
          } catch {
          }
        }
        function wA(J) {
          var u, _ = A.length, QA = 2147483648;
          if ((J >>>= 0) > QA)
            return !1;
          for (var T = 1; T <= 4; T *= 2) {
            var m = _ * (1 + 0.2 / T);
            if (m = Math.min(m, J + 100663296), eA(Math.min(QA, (u = Math.max(J, m)) + (65536 - u % 65536) % 65536)))
              return !0;
          }
          return !1;
        }
        x(j = "onig.wasm") || (j = a(j)), U = typeof dateNow < "u" ? dateNow : () => performance.now();
        var oA = [null, [], []];
        function sA(J, u) {
          var _ = oA[J];
          u === 0 || u === 10 ? ((J === 1 ? e : I)(B(_, 0)), _.length = 0) : _.push(u);
        }
        function CA(J, u, _, QA) {
          for (var T = 0, m = 0; m < _; m++) {
            var $ = E[u >> 2], tA = E[u + 4 >> 2];
            u += 8;
            for (var MA = 0; MA < tA; MA++)
              sA(J, A[$ + MA]);
            T += tA;
          }
          return E[QA >> 2] = T, 0;
        }
        var AA, iA = { emscripten_get_now: U, emscripten_memcpy_big: EA, emscripten_resize_heap: wA, fd_write: CA };
        function cA(J) {
          function u() {
            AA || (AA = !0, g.calledRun = !0, M || (S(), D(g), g.onRuntimeInitialized && g.onRuntimeInitialized(), W()));
          }
          v > 0 || (y(), v > 0 || (g.setStatus ? (g.setStatus("Running..."), setTimeout(function() {
            setTimeout(function() {
              g.setStatus("");
            }, 1), u();
          }, 1)) : u()));
        }
        if (GA(), g.___wasm_call_ctors = function() {
          return (g.___wasm_call_ctors = g.asm.__wasm_call_ctors).apply(null, arguments);
        }, g.___errno_location = function() {
          return (g.___errno_location = g.asm.__errno_location).apply(null, arguments);
        }, g._omalloc = function() {
          return (g._omalloc = g.asm.omalloc).apply(null, arguments);
        }, g._ofree = function() {
          return (g._ofree = g.asm.ofree).apply(null, arguments);
        }, g._getLastOnigError = function() {
          return (g._getLastOnigError = g.asm.getLastOnigError).apply(null, arguments);
        }, g._createOnigScanner = function() {
          return (g._createOnigScanner = g.asm.createOnigScanner).apply(null, arguments);
        }, g._freeOnigScanner = function() {
          return (g._freeOnigScanner = g.asm.freeOnigScanner).apply(null, arguments);
        }, g._findNextOnigScannerMatch = function() {
          return (g._findNextOnigScannerMatch = g.asm.findNextOnigScannerMatch).apply(null, arguments);
        }, g._findNextOnigScannerMatchDbg = function() {
          return (g._findNextOnigScannerMatchDbg = g.asm.findNextOnigScannerMatchDbg).apply(null, arguments);
        }, g.stackSave = function() {
          return (g.stackSave = g.asm.stackSave).apply(null, arguments);
        }, g.stackRestore = function() {
          return (g.stackRestore = g.asm.stackRestore).apply(null, arguments);
        }, g.stackAlloc = function() {
          return (g.stackAlloc = g.asm.stackAlloc).apply(null, arguments);
        }, g.dynCall_jiji = function() {
          return (g.dynCall_jiji = g.asm.dynCall_jiji).apply(null, arguments);
        }, g.UTF8ToString = s, b = function J() {
          AA || cA(), AA || (b = J);
        }, g.preInit)
          for (typeof g.preInit == "function" && (g.preInit = [g.preInit]); g.preInit.length > 0; )
            g.preInit.pop()();
        return cA(), N.ready;
      });
      K.exports = Q;
    } }, r = {}, function K(Q) {
      var N = r[Q];
      if (N !== void 0)
        return N.exports;
      var D = r[Q] = { exports: {} };
      return L[Q].call(D.exports, D, D.exports, K), D.exports;
    }(770);
    var L, r;
  });
})(CB);
var dA = CB.exports;
const QQ = /* @__PURE__ */ XA(dA), gQ = /* @__PURE__ */ gB({
  __proto__: null,
  default: QQ
}, [dA]);
var hA, nA;
let uA = (nA = class extends LA {
  constructor(c, L, r, K, Q, N, D, F, g, G) {
    super(), this._languageService = c, this._themeService = L, this._extensionResourceLoaderService = r, this._notificationService = K, this._logService = Q, this._configurationService = N, this._progressService = D, this._environmentService = F, this._instantiationService = g, this._telemetryService = G, this._createdModes = [], this._encounteredLanguages = [], this._debugMode = !1, this._debugModePrintFunc = () => {
    }, this._grammarDefinitions = null, this._grammarFactory = null, this._tokenizersRegistrations = new rA(), this._currentTheme = null, this._currentTokenColorMap = null, this._threadedBackgroundTokenizerFactory = this._instantiationService.createInstance(SA, (i, t, Y, a, k) => this._reportTokenizationTime(i, t, Y, a, !0, k), () => this.getAsyncTokenizationEnabled()), this._vscodeOniguruma = null, this._styleElement = rB(), this._styleElement.className = "vscode-tokens-styles", IA.setHandler((i) => this._handleGrammarsExtPoint(i)), this._updateTheme(this._themeService.getColorTheme(), !0), this._register(this._themeService.onDidColorThemeChange(() => {
      this._updateTheme(this._themeService.getColorTheme(), !1);
    })), this._languageService.onDidRequestRichLanguageFeatures((i) => {
      this._createdModes.push(i);
    });
  }
  getAsyncTokenizationEnabled() {
    return !!this._configurationService.getValue("editor.experimental.asyncTokenization");
  }
  getAsyncTokenizationVerification() {
    return !!this._configurationService.getValue("editor.experimental.asyncTokenizationVerification");
  }
  _handleGrammarsExtPoint(c) {
    this._grammarDefinitions = null, this._grammarFactory && (this._grammarFactory.dispose(), this._grammarFactory = null), this._tokenizersRegistrations.clear(), this._grammarDefinitions = [];
    for (const L of c) {
      const r = L.value;
      for (const K of r) {
        const Q = this._validateGrammarDefinition(L, K);
        if (Q && (this._grammarDefinitions.push(Q), Q.language)) {
          const N = new yB(() => this._createTokenizationSupport(Q.language));
          this._tokenizersRegistrations.add(N), this._tokenizersRegistrations.add(UA.registerFactory(Q.language, N));
        }
      }
    }
    this._threadedBackgroundTokenizerFactory.setGrammarDefinitions(this._grammarDefinitions);
    for (const L of this._createdModes)
      UA.getOrCreate(L);
  }
  _validateGrammarDefinition(c, L) {
    if (!IQ(c.description.extensionLocation, L, c.collector, this._languageService))
      return null;
    const r = AB(c.description.extensionLocation, L.path), K = /* @__PURE__ */ Object.create(null);
    if (L.embeddedLanguages) {
      const F = Object.keys(L.embeddedLanguages);
      for (let g = 0, G = F.length; g < G; g++) {
        const i = F[g], t = L.embeddedLanguages[i];
        typeof t == "string" && this._languageService.isRegisteredLanguageId(t) && (K[i] = this._languageService.languageIdCodec.encodeLanguageId(t));
      }
    }
    const Q = /* @__PURE__ */ Object.create(null);
    if (L.tokenTypes) {
      const F = Object.keys(L.tokenTypes);
      for (const g of F)
        switch (L.tokenTypes[g]) {
          case "string":
            Q[g] = 2;
            break;
          case "other":
            Q[g] = 0;
            break;
          case "comment":
            Q[g] = 1;
            break;
        }
    }
    const N = L.language && this._languageService.isRegisteredLanguageId(L.language) ? L.language : null;
    function D(F, g) {
      return !Array.isArray(F) || !F.every((G) => typeof G == "string") ? g : F;
    }
    return {
      location: r,
      language: N || void 0,
      scopeName: L.scopeName,
      embeddedLanguages: K,
      tokenTypes: Q,
      injectTo: L.injectTo,
      balancedBracketSelectors: D(L.balancedBracketScopes, ["*"]),
      unbalancedBracketSelectors: D(L.unbalancedBracketScopes, []),
      sourceExtensionId: c.description.id
    };
  }
  startDebugMode(c, L) {
    if (this._debugMode) {
      this._notificationService.error(z(
        "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
        "alreadyDebugging",
        "Already Logging."
      ));
      return;
    }
    this._debugModePrintFunc = c, this._debugMode = !0, this._debugMode && this._progressService.withProgress({
      location: 15,
      buttons: [z(
        "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
        "stop",
        "Stop"
      )]
    }, (r) => (r.report({
      message: z(
        "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
        "progress1",
        "Preparing to log TM Grammar parsing. Press Stop when finished."
      )
    }), this._getVSCodeOniguruma().then((K) => (K.setDefaultDebugCall(!0), r.report({
      message: z(
        "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
        "progress2",
        "Now logging TM Grammar parsing. Press Stop when finished."
      )
    }), new Promise((Q, N) => {
    })))), (r) => {
      this._getVSCodeOniguruma().then((K) => {
        this._debugModePrintFunc = () => {
        }, this._debugMode = !1, K.setDefaultDebugCall(!1), L();
      });
    });
  }
  _canCreateGrammarFactory() {
    return !!this._grammarDefinitions;
  }
  async _getOrCreateGrammarFactory() {
    if (this._grammarFactory)
      return this._grammarFactory;
    const c = await this._getVSCodeOniguruma(), L = Promise.resolve({
      createOnigScanner: (r) => c.createOnigScanner(r),
      createOnigString: (r) => c.createOnigString(r)
    });
    return this._grammarFactory ? this._grammarFactory : (this._grammarFactory = new BQ({
      logTrace: (r) => this._logService.trace(r),
      logError: (r, K) => this._logService.error(r, K),
      readFile: (r) => this._extensionResourceLoaderService.readExtensionResource(r)
    }, this._grammarDefinitions || [], jB, L), this._updateTheme(this._themeService.getColorTheme(), !0), this._grammarFactory);
  }
  async _createTokenizationSupport(c) {
    if (!this._languageService.isRegisteredLanguageId(c) || !this._canCreateGrammarFactory())
      return null;
    try {
      const L = await this._getOrCreateGrammarFactory();
      if (!L.has(c))
        return null;
      const r = this._languageService.languageIdCodec.encodeLanguageId(c), K = await L.createGrammar(c, r);
      if (!K.grammar)
        return null;
      const Q = DQ("editor.maxTokenizationLineLength", c, -1, this._configurationService), N = new WB(
        K.grammar,
        K.initialState,
        K.containsEmbeddedLanguages,
        (D, F) => this._threadedBackgroundTokenizerFactory.createBackgroundTokenizer(D, F, Q),
        () => this.getAsyncTokenizationVerification(),
        (D, F, g) => {
          this._reportTokenizationTime(D, c, K.sourceExtensionId, F, !1, g);
        },
        !0
      );
      return N.onDidEncounterLanguage((D) => {
        if (!this._encounteredLanguages[D]) {
          const F = this._languageService.languageIdCodec.decodeLanguageId(D);
          this._encounteredLanguages[D] = !0, this._languageService.requestBasicLanguageFeatures(F);
        }
      }), new XB(r, N, Q);
    } catch (L) {
      return L.message && L.message === kA || RB(L), null;
    }
  }
  _updateTheme(c, L) {
    var Q;
    if (!L && this._currentTheme && this._currentTokenColorMap && CQ(this._currentTheme.settings, c.tokenColors) && KB(this._currentTokenColorMap, c.tokenColorMap))
      return;
    this._currentTheme = { name: c.label, settings: c.tokenColors }, this._currentTokenColorMap = c.tokenColorMap, (Q = this._grammarFactory) == null || Q.setTheme(this._currentTheme, this._currentTokenColorMap);
    const r = EQ(this._currentTokenColorMap), K = YB(r);
    this._styleElement.textContent = K, UA.setColorMap(r), this._currentTheme && this._currentTokenColorMap && this._threadedBackgroundTokenizerFactory.acceptTheme(this._currentTheme, this._currentTokenColorMap);
  }
  async createTokenizer(c) {
    if (!this._languageService.isRegisteredLanguageId(c))
      return null;
    const L = await this._getOrCreateGrammarFactory();
    if (!L.has(c))
      return null;
    const r = this._languageService.languageIdCodec.encodeLanguageId(c), { grammar: K } = await L.createGrammar(c, r);
    return K;
  }
  _getVSCodeOniguruma() {
    return this._vscodeOniguruma || (this._vscodeOniguruma = (async () => {
      const c = await this._loadVSCodeOnigurumaWASM();
      return await dA.loadWASM({
        data: c,
        print: (L) => {
          this._debugModePrintFunc(L);
        }
      }), gQ;
    })()), this._vscodeOniguruma;
  }
  async _loadVSCodeOnigurumaWASM() {
    return ZA ? await (await fetch(aA.asBrowserUri("vscode-oniguruma/../onig.wasm").toString(!0))).arrayBuffer() : await fetch(this._environmentService.isBuilt ? aA.asBrowserUri(`${UB}/vscode-oniguruma/release/onig.wasm`).toString(!0) : aA.asBrowserUri(`${JA}/vscode-oniguruma/release/onig.wasm`).toString(!0));
  }
  _reportTokenizationTime(c, L, r, K, Q, N) {
    const D = Q ? "async" : "sync";
    hA.reportTokenizationTimeCounter[D] > 50 || (hA.reportTokenizationTimeCounter[D] === 0 && setTimeout(() => {
      hA.reportTokenizationTimeCounter[D] = 0;
    }, 1e3 * 60 * 60), hA.reportTokenizationTimeCounter[D]++, this._telemetryService.publicLog2("editor.tokenizedLine", {
      timeMs: c,
      languageId: L,
      lineLength: K,
      fromWorker: Q,
      sourceExtensionId: r,
      isRandomSample: N,
      tokenizationSetting: this.getAsyncTokenizationEnabled() ? this.getAsyncTokenizationVerification() ? 2 : 1 : 0
    }));
  }
}, hA = nA, nA.reportTokenizationTimeCounter = { sync: 0, async: 0 }, nA);
uA = hA = pA([
  V(0, jA),
  V(1, SB),
  V(2, zA),
  V(3, _A),
  V(4, uB),
  V(5, VA),
  V(6, pB),
  V(7, dB),
  V(8, BB),
  V(9, $A)
], uA);
function EQ(p) {
  const c = [null];
  for (let L = 1, r = p.length; L < r; L++)
    c[L] = PB.fromHex(p[L]);
  return c;
}
function CQ(p, c) {
  if (!c || !p || c.length !== p.length)
    return !1;
  for (let L = c.length - 1; L >= 0; L--) {
    const r = c[L], K = p[L];
    if (r.scope !== K.scope)
      return !1;
    const Q = r.settings, N = K.settings;
    if (Q && N) {
      if (Q.fontStyle !== N.fontStyle || Q.foreground !== N.foreground || Q.background !== N.background)
        return !1;
    } else if (!Q || !N)
      return !1;
  }
  return !0;
}
function IQ(p, c, L, r) {
  if (c.language && (typeof c.language != "string" || !r.isRegisteredLanguageId(c.language)))
    return L.error(z(
      "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
      "invalid.language",
      "Unknown language in `contributes.{0}.language`. Provided value: {1}",
      IA.name,
      String(c.language)
    )), !1;
  if (!c.scopeName || typeof c.scopeName != "string")
    return L.error(z(
      "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
      "invalid.scopeName",
      "Expected string in `contributes.{0}.scopeName`. Provided value: {1}",
      IA.name,
      String(c.scopeName)
    )), !1;
  if (!c.path || typeof c.path != "string")
    return L.error(z(
      "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
      "invalid.path.0",
      "Expected string in `contributes.{0}.path`. Provided value: {1}",
      IA.name,
      String(c.path)
    )), !1;
  if (c.injectTo && (!Array.isArray(c.injectTo) || c.injectTo.some((Q) => typeof Q != "string")))
    return L.error(z(
      "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
      "invalid.injectTo",
      "Invalid value in `contributes.{0}.injectTo`. Must be an array of language scope names. Provided value: {1}",
      IA.name,
      JSON.stringify(c.injectTo)
    )), !1;
  if (c.embeddedLanguages && !qA(c.embeddedLanguages))
    return L.error(z(
      "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
      "invalid.embeddedLanguages",
      "Invalid value in `contributes.{0}.embeddedLanguages`. Must be an object map from scope name to language. Provided value: {1}",
      IA.name,
      JSON.stringify(c.embeddedLanguages)
    )), !1;
  if (c.tokenTypes && !qA(c.tokenTypes))
    return L.error(z(
      "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
      "invalid.tokenTypes",
      "Invalid value in `contributes.{0}.tokenTypes`. Must be an object map from scope name to token type. Provided value: {1}",
      IA.name,
      JSON.stringify(c.tokenTypes)
    )), !1;
  const K = AB(p, c.path);
  return JB(K, p) || L.warn(z(
    "vs/workbench/services/textMate/browser/textMateTokenizationFeatureImpl",
    "invalid.path.1",
    "Expected `contributes.{0}.path` ({1}) to be included inside extension's folder ({2}). This might make the extension non-portable.",
    IA.name,
    K.path,
    p.path
  )), !0;
}
function DQ(p, c, L, r) {
  return vA((K) => r.onDidChangeConfiguration((Q) => {
    Q.affectsConfiguration(p, { overrideIdentifier: c }) && K(Q);
  }), () => r.getValue(p, { overrideIdentifier: c }) ?? L);
}
const bA = new URL("data:application/wasm;base64,", import.meta.url).href;
lB({
  "vscode-oniguruma/../onig.wasm": bA,
  "vs/../../node_modules/vscode-oniguruma/release/onig.wasm": bA
});
mB(async (p) => {
  p.get(bB).when(2).then(() => {
    TB.get(QB);
  });
});
let NA = class {
  constructor(c) {
    this.instantiationService = c, this.instantiationService.createInstance(qB);
  }
};
NA.ID = "workbench.contrib.extensionPoints.textmate";
NA = pA([
  V(0, BB)
], NA);
fB(NA.ID, NA, 1);
function sQ() {
  return {
    ...OB(),
    [QB.toString()]: new xB(uA, [], !1)
  };
}
export {
  QB as ITextMateTokenizationService,
  sQ as default
};
//# sourceMappingURL=index-Bv4NTDxD.js.map
