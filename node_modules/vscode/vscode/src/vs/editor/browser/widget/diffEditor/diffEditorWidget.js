...
import { ObservableElementSizeObserver, bindContextKey, applyViewZones, applyStyle, translatePosition, readHotReloadableExport } from './utils.js';
...
import { observableValue, subtransaction, transaction } from '../../../../base/common/observableInternal/base.js';
    ...
    constructor(_domElement, options, codeEditorWidgetOptions, _parentContextKeyService, _parentInstantiationService, codeEditorService, _accessibilitySignalService, _editorProgressService) {
        ...
        this._register(applyStyle(this.elements.overlay, {
            width: ( this._layoutInfo.map(
                (i, r) => i.originalEditor.width + (this._options.renderSideBySide.read(r) ? 0 : i.modifiedEditor.width)
            )),
            visibility: derived(reader =>  (this._options.hideUnchangedRegions.read(reader) && this._diffModel.read(reader)?.diff.read(reader)?.mappings.length === 0)
                ? 'visible' : 'hidden'),
        }));
