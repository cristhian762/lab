...
let HideUnchangedRegionsFeature = class HideUnchangedRegionsFeature extends Disposable {
    ...
    static { this._breadcrumbsSourceFactory = observableValue('breadcrumbsSourceFactory', undefined); }
    ...
    constructor(_editors, _diffModel, _options, _instantiationService) {
        ...
        const unchangedLinesDecorationShow = {
            ...
            glyphMarginHoverMessage: ( new MarkdownString(undefined, { isTrusted: true, supportThemeIcons: true }))
                .appendMarkdown(( localizeWithPath(
...
class CollapsedCodeOverlayWidget extends ViewZoneOverlayWidget {
    constructor(_editor, _viewZone, _unchangedRegion, _unchangedRegionRange, _hide, _modifiedOutlineSource, _revealModifiedHiddenLine, _options) {
        ...
        this._nodes = h('div.diff-hidden-lines', [
            h('div.top@top', { title: ( localizeWithPath(
                'vs/editor/browser/widget/diffEditor/features/hideUnchangedRegionsFeature',
                'diff.hiddenLines.top',
                'Click or drag to show more above'
            )) }),
            h('div.center@content', { style: { display: 'flex' } }, [
                h('div@first', { style: { display: 'flex', justifyContent: 'center', alignItems: 'center', flexShrink: '0' } }, [$('a', { title: ( localizeWithPath(
                    'vs/editor/browser/widget/diffEditor/features/hideUnchangedRegionsFeature',
                    'showUnchangedRegion',
                    'Show Unchanged Region'
                )), role: 'button', onclick: () => { this._unchangedRegion.showAll(undefined); } }, ...renderLabelWithIcons('$(unfold)'))]),
                h('div@others', { style: { display: 'flex', justifyContent: 'center', alignItems: 'center' } }),
            ]),
            h('div.bottom@bottom', { title: ( localizeWithPath(
                'vs/editor/browser/widget/diffEditor/features/hideUnchangedRegionsFeature',
                'diff.bottom',
        ...
        this._register(addDisposableListener(this._nodes.top, 'mousedown', e => {
            ...
            e.preventDefault();
            this._unchangedRegion.visibleLineCountTop.get();
            this._unchangedRegion.isDragged.set('top', undefined);
            getWindow(this._nodes.top);
        }));
        this._register(addDisposableListener(this._nodes.bottom, 'mousedown', e => {
            ...
            this._nodes.root.classList.toggle('dragging', true);
            e.preventDefault();
            this._unchangedRegion.visibleLineCountBottom.get();
            this._unchangedRegion.isDragged.set('bottom', undefined);
            getWindow(this._nodes.bottom);
        }));
        this._register(autorun(reader => {
            const children = [];
            if (!this._hide) {
                const lineCount = _unchangedRegion.getHiddenModifiedRange(reader).length;
                const linesHiddenText = ( localizeWithPath(
                    'vs/editor/browser/widget/diffEditor/features/hideUnchangedRegionsFeature',
                    'hiddenLines',
                    '{0} hidden lines',
                    lineCount
                ));
                const span = $('span', { title: ( localizeWithPath(
                    'vs/editor/browser/widget/diffEditor/features/hideUnchangedRegionsFeature',
