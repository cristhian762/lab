...
import { registerEditorContribution, registerEditorAction, registerEditorCommand, registerModelAndPositionCommand, EditorCommand, EditorAction } from '../../../browser/editorExtensions.js';
...
import { RenameInputField, CONTEXT_RENAME_INPUT_VISIBLE, CONTEXT_RENAME_INPUT_FOCUSED } from './renameInputField.js';
...
class RenameSkeleton {
    ...
    async _provideRenameEdits(newName, i, rejects, token) {
        ...
        if (!result) {
            return this._provideRenameEdits(newName, i + 1, rejects.concat(( localizeWithPath('vs/editor/contrib/rename/browser/rename', 'no result', "No result."))), token);
        }
...
let RenameController = class RenameController {
    ...
    async run() {
        ...
        try {
            ...
            MessageController.get(this.editor)?.showMessage(e || ( localizeWithPath(
        ...
        const renameOperation = raceCancellation(skeleton.provideRenameEdits(inputFieldResult.newName, cts2.token), cts2.token).then(async (renameResult) => {
            ...
            this._bulkEditService.apply(renameResult, {
                ...
                quotableLabel: ( localizeWithPath(
                    'vs/editor/contrib/rename/browser/rename',
                    'quotableLabel',
                    "Renaming {0} to {1}",
                    loc?.text,
...
class RenameAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localizeWithPath('vs/editor/contrib/rename/browser/rename', 'rename.label', "Rename Symbol")),
            alias: 'Rename Symbol',
