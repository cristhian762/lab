...
import { EditorAction, registerEditorContribution, registerEditorAction } from '../../../browser/editorExtensions.js';
...
import { getSpaceCnt, generateIndent } from './indentUtils.js';
...
class ChangeIndentationSizeAction extends EditorAction {
    ...
    run(accessor, editor) {
        ...
        setTimeout(() => {
            quickInputService.pick(picks, { placeHolder: ( localizeWithPath(
                'vs/editor/contrib/indentation/browser/indentation',
                { key: 'selectTabWidth', comment: ['Tab corresponds to the tab key'] },
                "Select Tab Size for Current File"
            )), activeItem: picks[autoFocusIndex] }).then(pick => {
                if (pick) {
...
let AutoIndentOnPaste = class AutoIndentOnPaste {
    ...
    update() {
        ...
        if (this.editor.getOption(12 ) < 4  || this.editor.getOption(55 )) {
            return;
        }
