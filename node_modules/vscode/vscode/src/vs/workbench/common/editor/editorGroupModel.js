...
import { EditorCloseContext, SideBySideEditor, EditorExtensions } from '../editor.js';
...
function isGroupEditorOpenEvent(e) {
    const candidate = e;
    return candidate.kind === 4  && candidate.editorIndex !== undefined;
}
function isGroupEditorMoveEvent(e) {
    const candidate = e;
    return candidate.kind === 6  && candidate.editorIndex !== undefined && candidate.oldEditorIndex !== undefined;
}
function isGroupEditorCloseEvent(e) {
    const candidate = e;
    return candidate.kind === 5  && candidate.editorIndex !== undefined && candidate.context !== undefined && candidate.sticky !== undefined;
}
let EditorGroupModel = class EditorGroupModel extends Disposable {
    ...
    registerEditorListeners(editor) {
        ...
        listeners.add(this.onDidModelChange(event => {
            if (event.kind === 5  && event.editor?.matches(editor)) {
                dispose(listeners);
