...
class ExtHostNotebookController {
    ...
    async $saveNotebook(handle, uriComponents, versionId, options, token) {
        ...
        if (!this._extHostFileSystem.value.isWritableFileSystem(uri.scheme)) {
            throw new FileOperationError(localizeWithPath('vs/workbench/api/common/extHostNotebook', 'err.readonly', "Unable to modify read-only file '{0}'", this._resourceForError(uri)), 6 );
        }
    ...
    async _validateWriteFile(uri, options) {
        ...
        if (typeof options?.mtime === 'number' && typeof options.etag === 'string' && options.etag !== ETAG_DISABLED &&
            ...
            throw new FileOperationError(localizeWithPath('vs/workbench/api/common/extHostNotebook', 'fileModifiedError', "File Modified Since"), 3 , options);
        }
        return;
    }
