...
import { SimpleNotebookProviderInfo, INotebookService } from './notebookService.js';
...
let NotebookEditorInput = class NotebookEditorInput extends AbstractResourceEditorInput {
    ...
    getDescription(verbosity = 1 ) {
        if (!this.hasCapability(4 ) || this._editorModelReference?.object.hasAssociatedFilePath()) {
    ...
    isSaving() {
        ...
        if (!model || !model.isDirty() || model.hasErrorState || this.hasCapability(4 )) {
            return false;
        }
    ...
    async saveAs(group, options) {
        ...
        const pathCandidate = this.hasCapability(4 ) ? await this._suggestName(provider, this.labelService.getUriBasenameLabel(this.resource)) : this._editorModelReference.object.resource;
        let target;
