...
import { IExtensionsWorkbenchService, VIEWLET_ID } from '../../../extensions/common/extensions.js';
...
import { INotebookKernelService, INotebookKernelHistoryService } from '../../common/notebookKernelService.js';
...
function toKernelQuickPick(kernel, selected) {
    ...
    if (kernel.id === selected?.id) {
        if (!res.description) {
            res.description = ( localizeWithPath(
...
class KernelPickerStrategyBase {
    ...
    async _showKernelExtension(paneCompositePartService, extensionWorkbenchService, extensionService, viewType, extIds, isInsiders) {
        ...
        for (const extId of extIds) {
            ...
            if (extension.enablementState === 6  || extension.enablementState === 7  || extension.enablementState === 2 ) {
        ...
        if (extensionsToInstall.length || extensionsToEnable.length) {
            await Promise.all([...( extensionsToInstall.map(async (extension) => {
                    ...
                    switch (extension.enablementState) {
                        case 7 :
                            await extensionWorkbenchService.setEnablement([extension], 9 );
                            return;
                        case 6 :
                            await extensionWorkbenchService.setEnablement([extension], 8 );
                            return;
                        case 2 :
                            await extensionWorkbenchService.setEnablement([extension], 3 );
                            return;
                    }
                }))]);
    ...
    async _getKernelRecommendationsQuickPickItems(notebookTextModel, extensionWorkbenchService) {
        ...
        if (suggestedExtension) {
            ...
            const extensions = extensionWorkbenchService.installed.filter(e => ((e.enablementState === 3  || e.enablementState === 8  || e.enablementState === 9) )
            ...
            quickPickItems.push({
                ...
                label: `$(${Codicon.lightbulb.id}) ` + ( localizeWithPath(
                    'vs/workbench/contrib/notebook/browser/viewParts/notebookKernelQuickPickStrategy',
...
let KernelPickerMRUStrategy = class KernelPickerMRUStrategy extends KernelPickerStrategyBase {
    ...
    async displaySelectAnotherQuickPick(editor, kernelListEmpty) {
        ...
        const quickPickItem = await ( new Promise(resolve => {
            quickPick.title = kernelListEmpty ? ( localizeWithPath(
                'vs/workbench/contrib/notebook/browser/viewParts/notebookKernelQuickPickStrategy',
                'select',
                "Select Kernel"
            )) : ( localizeWithPath(
