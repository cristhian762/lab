...
import { SelectionStateType, CellKind } from '../../common/notebookCommon.js';
...
function runDeleteAction(editor, cell) {
    ...
    if (containingSelection) {
        const edits = ( selections.reverse().map(selection => ({
            editType: 1 , index: selection.start, count: selection.end - selection.start, cells: []
        })));
...
async function joinSelectedCells(bulkEditService, notificationService, context) {
    ...
    if (edits.length) {
        await bulkEditService.apply(edits, { quotableLabel: ( localizeWithPath(
            'vs/workbench/contrib/notebook/browser/controller/cellOperations',
...
function computeCellLinesContents(cell, splitPoints) {
    ...
    for (let i = 1; i < rangeBoundaries.length; i++) {
        ...
        newLineModels.push(cell.textBuffer.getValueInRange(( new Range(start.lineNumber, start.column, end.lineNumber, end.column)), 0 ));
    }
