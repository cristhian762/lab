...
import { basenameOrAuthority, joinPath, basename as basename$1, isEqualOrParent, distinctParents } from '../../../../base/common/resources.js';
...
import { EmptyWorkspaceSupportContext, ActiveEditorContext, ActiveEditorCanToggleReadonlyContext } from '../../../common/contextkeys.js';
...
const NEW_FILE_LABEL = ( localize2WithPath('vs/workbench/contrib/files/browser/fileActions', 'newFile', "New File..."));
const NEW_FOLDER_COMMAND_ID = 'explorer.newFolder';
const NEW_FOLDER_LABEL = ( localize2WithPath(
    'vs/workbench/contrib/files/browser/fileActions',
    'newFolder',
    "New Folder..."
));
const TRIGGER_RENAME_LABEL = ( localizeWithPath('vs/workbench/contrib/files/browser/fileActions', 'rename', "Rename..."));
const MOVE_FILE_TO_TRASH_LABEL = ( localizeWithPath('vs/workbench/contrib/files/browser/fileActions', 'delete', "Delete"));
const COPY_FILE_LABEL = ( localizeWithPath('vs/workbench/contrib/files/browser/fileActions', 'copyFile', "Copy"));
const PASTE_FILE_LABEL = ( localizeWithPath('vs/workbench/contrib/files/browser/fileActions', 'pasteFile', "Paste"));
const FileCopiedContext = ( new RawContextKey('fileCopied', false));
const DOWNLOAD_COMMAND_ID = 'explorer.download';
const DOWNLOAD_LABEL = ( localizeWithPath(
    'vs/workbench/contrib/files/browser/fileActions',
    'download',
    "Download..."
));
const UPLOAD_COMMAND_ID = 'explorer.upload';
const UPLOAD_LABEL = ( localizeWithPath('vs/workbench/contrib/files/browser/fileActions', 'upload', "Upload..."));
...
async function deleteFiles(explorerService, workingCopyFileService, dialogService, configurationService, elements, useTrash, skipConfirm = false, ignoreIfNotExists = false) {
    let primaryButton;
    if (useTrash) {
        primaryButton = isWindows ? ( localizeWithPath(
            'vs/workbench/contrib/files/browser/fileActions',
    ...
    let confirmation;
    const deleteDetail = ( distinctElements.some(e => e.isDirectory)) ? ( localizeWithPath(
        'vs/workbench/contrib/files/browser/fileActions',
        'irreversible',
        "This action is irreversible!"
    )) :
        distinctElements.length > 1 ? ( localizeWithPath(
            'vs/workbench/contrib/files/browser/fileActions',
            'restorePlural',
    ...
    if (skipConfirm || (useTrash && configurationService.getValue(CONFIRM_DELETE_SETTING_KEY) === false)) {
        ...
        detail += detail ? '\n' : '';
        if (isWindows) {
            detail += distinctElements.length > 1 ? ( localizeWithPath(
                'vs/workbench/contrib/files/browser/fileActions',
                'undoBinFiles',
    ...
    try {
        ...
        const options = {
            undoLabel: distinctElements.length > 1 ? ( localizeWithPath(
                'vs/workbench/contrib/files/browser/fileActions',
            ...
            progressLabel: distinctElements.length > 1 ? ( localizeWithPath(
                'vs/workbench/contrib/files/browser/fileActions',
        ...
        let primaryButton;
        if (useTrash) {
            errorMessage = isWindows ? ( localizeWithPath(
                'vs/workbench/contrib/files/browser/fileActions',
                'binFailed',
...
function getMoveToTrashMessage(distinctElements) {
    ...
    if (distinctElements[0].isDirectory && !distinctElements[0].isSymbolicLink) {
        return { message: ( localizeWithPath(
            'vs/workbench/contrib/files/browser/fileActions',
...
function onErrorWithRetry(notificationService, error, retry) {
    notificationService.prompt(Severity.Error, toErrorMessage(error, false), [{
            label: ( localizeWithPath('vs/workbench/contrib/files/browser/fileActions', 'retry', "Retry")),
            run: () => retry()
        }]);
}
...
const pasteFileHandler = async (accessor, fileList) => {
    ...
    if (confirmPasteNative && toPaste.files.length >= 1) {
        ...
        const detail = toPaste.files.length > 1 ? getFileNamesMessage(( toPaste.files.map(item => toPaste.type === 'paths' ? item.path : item.name))) : undefined;
    ...
    try {
        ...
        if (toPaste.type === 'paths') {
            const sourceTargetPairs = coalesce(await Promise.all(( toPaste.files.map(async (fileToPaste) => {
                if (( element.resource.toString()) !== ( fileToPaste.toString()) && isEqualOrParent(element.resource, fileToPaste)) {
                    throw new Error(localizeWithPath('vs/workbench/contrib/files/browser/fileActions', 'fileIsAncestor', "File to paste is an ancestor of the destination folder"));
                }
            ...
            if (sourceTargetPairs.length >= 1) {
                if (pasteShouldMove) {
                    ...
                    const options = {
                        confirmBeforeUndo: configurationService.getValue().explorer.confirmUndo === "verbose" ,
                        progressLabel: sourceTargetPairs.length > 1 ? ( localizeWithPath(
                            'vs/workbench/contrib/files/browser/fileActions',
                            ...
                            "Moving {0}",
                            basenameOrAuthority(sourceTargetPairs[0].target)
                        )),
                        undoLabel: sourceTargetPairs.length > 1 ? ( localizeWithPath(
                            'vs/workbench/contrib/files/browser/fileActions',
        ...
        onError(notificationService, ( new Error(( localizeWithPath(
            'vs/workbench/contrib/files/browser/fileActions',
            'fileDeleted',
    ...
    async function applyCopyResourceEdit(targets, resourceFileEdits) {
        ...
        const options = {
            ...
            progressLabel: targets.length > 1 ? ( localizeWithPath(
                'vs/workbench/contrib/files/browser/fileActions',
                ...
                "Copying {0}",
                basenameOrAuthority(targets[0])
            )),
            undoLabel: targets.length > 1 ? ( localizeWithPath(
                'vs/workbench/contrib/files/browser/fileActions',
