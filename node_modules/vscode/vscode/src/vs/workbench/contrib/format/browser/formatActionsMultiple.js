...
import { FormattingConflicts, getRealAndSyntheticDocumentFormattersOrdered, formatDocumentWithProvider, formatDocumentRangesWithProvider } from '../../../../editor/contrib/format/browser/format.js';
...
import { Severity, NotificationPriority, INotificationService } from '../../../../platform/notification/common/notification.js';
...
let DefaultFormatter = class DefaultFormatter extends Disposable {
    ...
    async _updateConfigValues() {
        ...
        DefaultFormatter_1.extensionItemLabels.push(( localizeWithPath(
            'vs/workbench/contrib/format/browser/formatActionsMultiple',
            'null',
            'None'
        )));
        DefaultFormatter_1.extensionDescriptions.push(( localizeWithPath(
            'vs/workbench/contrib/format/browser/formatActionsMultiple',
    ...
    async _selectFormatter(formatter, document, mode, kind) {
        ...
        if (mode !== 2 ) {
            ...
            this._notificationService.prompt(Severity.Info, formatterOrMessage, [{ label: ( localizeWithPath(
                'vs/workbench/contrib/format/browser/formatActionsMultiple',
    ...
    async _pickAndPersistDefaultFormatter(formatter, document) {
        ...
        const langName = this._languageService.getLanguageName(document.getLanguageId()) || document.getLanguageId();
        const pick = await this._quickInputService.pick(picks, { placeHolder: ( localizeWithPath(
            'vs/workbench/contrib/format/browser/formatActionsMultiple',
            'select',
    ...
    _updateStatus() {
        ...
        this._analyzeFormatter(1 , formatter, document).then(result => {
            ...
            const command = { id: `formatter/configure/dfl/${generateUuid()}`, title: ( localizeWithPath(
                'vs/workbench/contrib/format/browser/formatActionsMultiple',
                'do.config.command',
                "Configure..."
            )) };
...
async function showFormatterPick(accessor, model, formatters) {
    ...
    const picks = ( formatters.map((provider, index) => {
        ...
        const pick = {
            ...
            description: isDefault ? ( localizeWithPath(
                'vs/workbench/contrib/format/browser/formatActionsMultiple',
                'def',
                "(default)"
            )) : undefined,
    ...
    if (!pick) {
        ...
        const pick = await quickPickService.pick(picks, { placeHolder: ( localizeWithPath(
            'vs/workbench/contrib/format/browser/formatActionsMultiple',
            'select',
            "Select a default formatter for '{0}' files",
